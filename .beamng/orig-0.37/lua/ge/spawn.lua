...
local function getBBOnGround(vehBB, vehOrigin)
  ...
  if moveBBToGround(newBB, true) then
    -- if the first moveBBToGround call doesnt hit anything with the raycast, it might be that it is slightly underground, so we try again with a higher position
    ...
    if not moveBBToGround(newBBHigher, true) then
