...
local function jetColorF(x, a)
  ...
  return ColorF(r, g, b, a or 0.85)
end

local function getColorForIndex(index, total, alpha)
  local denom = math.max(1, (total or 1) - 1)
  local t = ((index or 1) - 1) / denom
  return jetColorF(t, alpha or 0.85)
end

-- Draw all vehicle nodes colored by their cluster
local function drawVehicleNodeClusters(veh)
  veh = veh or getPlayerVehicle(0)
  if not veh then return end

  local nodeCount = veh:getNodeCount()
  if not nodeCount or nodeCount <= 0 then return end
  ...
  -- Map unique cluster ids to a color index
  local clusterIndexToColorIndex = {}
  for i, clusterId in ipairs(clusters) do
    clusterIndexToColorIndex[clusterId] = i
  end

  for nodeId = 0, nodeCount - 1 do
    local cid = clusters[nodeId + 1]
    local colorIndex = clusterIndexToColorIndex[cid] or 1
    local col = getColorForIndex(colorIndex, #clusters, 0.85)
    local pos = veh:getNodePosition(nodeId) + vehiclePos
    debugDrawer:drawSphere(pos, r, col)
  end
end

local function setDrawNodeClusters(enabled)
  drawNodeClusters = enabled
end

local function onUpdate()
  spawn.clearCache()

  if drawNodeClusters then
    drawVehicleNodeClusters()
  end
end
...
M.setDrawNodeClusters = setDrawNodeClusters
