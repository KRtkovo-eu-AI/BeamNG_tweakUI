...
local M = {}
M.dependencies = { 'core_vehiclePaints' }
...
local function computeFilesCaches()
  ...
  for _, fn in ipairs(filesPaints) do
    ...
    if data then
      data = jsonDecode(data)
      if not data then
        log('E', 'vehicles', 'unable to read paints file, ignoring: '.. fn)
      else
        filesParsed[fn] = data
      end
    end
...
local function _modelConfigsHelper(key, model, ignoreCache)
  ...
  for configFilename, configName in pairs(vehFiles.configs) do
    ...
    local mod = extensions.core_modmanager.getModFromPath(configFilename, true) -- TODO: FIXME: SUPER SLOW
    if mod then
...
local function openSelectorUI()
  ...
  ui_vehicleSelector.openVehicleSelectorForFreeroam()
end
