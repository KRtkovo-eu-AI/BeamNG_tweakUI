...
function C:update(data)
  ...
  if self.ctrlPoint ~= self.lastFrameCtrlPoint then
    ...
    if markers[n2].cut then
      data.res.pos:set(markers[n3].pos)
      data.res.rot:set(markers[n3].rot)
  ...
  if data.pos then
    ...
    -- Apply tracking offset if available (from camPathEditor)
    if data.veh and extensions and extensions.editor_camPathEditor then
  ...
  -- application
  data.res.pos:set(pos)
  data.res.rot:set(rot)
  if path.manualFov then
    if self.fovOffset > 0 then
