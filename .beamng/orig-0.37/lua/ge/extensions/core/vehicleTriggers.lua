...
local function executeLink(vdata, lnk, actionValue, vehicleId)
  if lnk.version and lnk.version == 2 then
    ...
    if lnk.namespace == 'vehicle' then
      ...
      return core_input_actions.executeCommand(vdata.inputActions[lnk.inputAction], actionValue, vehicleId)
...
local function onUpdate(dtReal, dtSim, dtRaw)
  ...
  if currentlyUsedTrigger then
    ...
    if fpsLimiter:update(dtReal) then
      ...
      local hit = be:triggerRaycastClosest(maxTriggerDistance, useCursorCoordinates)
      if (hit and hit.v) ~= hoveredTriggerId.vehicleId or (hit and hit.t) ~= hoveredTriggerId.triggerId then
        if hit then
          local vData = extensions.core_vehicle_manager.getVehicleData(hit.v)
          if vData and vData.vdata and type(vData.vdata.triggers) == 'table' then
            local trigger = vData.vdata.triggers[hit.t]
            if trigger then
              local actionName = vData.vdata.triggerEventLinksDict[hit.t]["action0"][1].inputAction
              ...
              if action then
                ...
                ui_bindingsLegend.addActions("vehicleTriggers", {{action = actionStr, label = action.title}}, {priority = 6})
              end
            end
          end
        else
