...
local function getInfo()
  ...
  if fb == 'win7' then
    ...
    if not warningDisplayed.win7 then
      print("win 7 warn")
      warningDisplayed.win7 = true
    end
  ...
  if res.os.type == 'linux' then
    if res.os.shortname == "Ubuntu" then
      if res.os.versionMajor < 22 or res.os.versionMajor > 24 then
    ...
    elseif res.os.shortname ~= "SteamOS" and beamng_appname == "BeamNG.drive" then
      ...
      if not warningDisplayed.unsupportedlinuxdist then
        log("W", "core_hwinfo", "unsupported linux distribution\n" .. dumps(res.os))
        warningDisplayed.unsupportedlinuxdist = true
      end
    end

    if res.os.windowsystem == 'wayland' and not warningDisplayed.wayland then
      -- https://github.com/ocornut/imgui/issues/8587
      local msg = "Running under Wayland, there are known issues with imgui viewports."
      log("W", "core_hwinfo", msg)
      warningDisplayed.wayland = true
    end
  end
