...
local options = {
  ...
  rallyCountdownStyle = {modes={keys={'countdown_style_3', 'countdown_style_5'}, values={'ui.options.rally.countdown.fromThree', 'ui.options.rally.countdown.fromFive'}}},
...
local function refreshRallyVoices()
  local compositorsDir = '/lua/ge/extensions/gameplay/rally/compositors/'
  local compositorDirs = FS:directoryList(compositorsDir, false, true)
  ...
  for _, compositorPath in ipairs(compositorDirs) do
    ...
    for _, voiceDir in ipairs(voiceDirs) do
      ...
      -- hide voice by naming the folder with leading underscore or _raw prefix
      if basenameVoice and not basenameVoice:match("_raw$") and not basenameVoice:match("^_") then
  ...
  options.rallyTextCompositorVoice = { modes={ keys=keys, values=values } }
  notifyUI()
end
