...
local function createPathActionUndo(actionData)
  if actionData.path.id == M.currentPath.id then
    selectPath(nil)
  end
  ...
  for i = 2, #path.markers - 1 do
    ...
    if not prevMarker.cut and not currentMarker.cut then
      ...
      if totalDist > 0 then
        ...
        local idealTimeNext = totalTime * (distNext / totalDist)
...
local function displayViewBig(marker, dist, frustumColor, frustumColorI)
  ...
  debugDrawer:drawLineInstance(pos, d, lineWidth, frustumColor)
  debugDrawer:drawLineInstance(a, b, lineWidth, frustumColor)
  debugDrawer:drawLineInstance(b, c, lineWidth, frustumColor)
...
local function drawDebugPath(path, focusPos)
  ...
  for index, marker in ipairs(markers) do
    ...
    if hoveredPath and hoveredPath.id == path.id and hoveredMarker == index then
      color = roadRiverGui.highlightColors.hoveredNode
    end
    if M.currentPath and (M.currentPath.id == path.id) and currentMarkerIndex == index then
    ...
    color.a = math.min(math.max(0, (marker.pos - core_camera.getPosition()):length() - markerVisibleRadius), 10) / 10
    if not (M.currentPath and path.di == M.currentPath.id) then
      color.a = color.a * 0.2
    end
