...
local sortedByIndex = {}
local sortedByName = {}
local filterResultItemIndex = 0
local filterValueChanged = false
...
local function itemGui(cat, subCat, item, itemIndex)
  local fieldValue = editor.preferencesRegistry:getAsString(item.path)
  local desc = item.description

  if not desc or desc == "" then desc = "<No description>" end

  local defVal = editor.preferencesRegistry:itemValueToString(item, item.defaultValue)
  ...
  valueInspector:valueEditorGui(item.name, fieldValue or "", itemIndex or 0, item.label, desc, item.type or "", item.typeName or "", item, pastePrefValue, contextMenuUI)
end
  ...
  for _, subCat in ipairs(cat.subcategories) do
    ...
    for i = 1, #subCat.items do
      ...
      then
        ...
        if filterValueChanged then
          filterResultItemIndex = filterResultItemIndex + 1
          subCat.items[i].itemIndex = filterResultItemIndex
        end

      else
        subCat.items[i].visibleOnSearch = false
        subCat.items[i].itemIndex = nil
      end
    end
  end
    ...
    if subCat.visibleOnSearch and imgui.CollapsingHeader1(subCat.label, nodeFlags) then
      ...
      for i = 1, #subCat.items do
        if subCat.items[i].visibleOnSearch and not subCat.items[i].hidden then
          itemGui(cat, subCat, subCat.items[i], subCat.items[i].itemIndex)
        end
      end
      imgui.PopID()
      imgui.Unindent(15)
...
local function onEditorGui()
  if editor.beginWindow(toolWindowName, "Preferences") then
    ...
    if filterValueChanged then filterResultItemIndex = 0 end
...
local function onEditorPreferenceValueChanged(path, value)
  ...
  if path == "camera.general.freeCameraMoveSpeed" then editor.setCameraSpeed(value) end
...
local function onEditorRegisterPreferences(prefsRegistry)
  ...
  {
    -- {name = {type, default value, desc, label (nil for auto Sentence Case), min, max, hidden, advanced, customUiFunc, enumLabels, prefVersion}}
    -- hidden
  ...
  prefsRegistry:registerSubCategory("camera", "general", nil,
  {
    ...
    {freeCameraMoveSpeed = {"float", 30, "The free camera's move speed", nil, 1, 100}},
...
local function onEditorDeactivated()
  ...
  if editor.getPreference("camera.general.setVehicleCameraWhenExitingEditor") and not editor.isGameFreeCamera and not editor.skipCameraOnExit then
    editor.selectCamera(editor.CameraType_Game)
  end
  editor.skipCameraOnExit = false
end
