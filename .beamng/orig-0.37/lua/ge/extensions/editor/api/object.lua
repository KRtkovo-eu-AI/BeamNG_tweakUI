...
local function copyFields(objectID)
  ...
  local index = 1
  while index <= #sortedFieldData do
...
local function pasteFieldsToObject(fields, targetObject)
  if fields.fields then
    for fieldName, value in pairs(fields.fields) do
...
local function createPrefabFromObjectSelection(newPrefabFilename, objName, loadmode, pathNative)
  ...
  -- now register prefab
  prefab:registerObject(objName)
  parentGroup:addObject(prefab)
  prefab:setChildGroup(objName, group); -- prefab needs to be registered before we can set the child group because we have a map using the prefab id

  local success = prefab:save(fname, false)
  if not success then
...
local function initialize(editorInstance)
  ...
  editor.pasteFieldsToObject = pasteFieldsToObject
