...
local function setFilenameContextMenu(fieldName, fieldType, fieldValue, arrayIndex, customData, pasteCallback, contextMenuUI)
  if imgui.IsItemHovered() and imgui.IsMouseClicked(1) then
    ...
    filenameContextMenu.fieldType = fieldType
    filenameContextMenu.fieldValue = fieldValue
    filenameContextMenu.arrayIndex = arrayIndex
    filenameContextMenu.customData = customData
    filenameContextMenu.pasteCallback = pasteCallback -- pasteCallback(fieldName, fieldValue, arrayIndex, customData)
    filenameContextMenu.contextMenuUI = contextMenuUI
  end
end
...
local function filenameContextMenuGui()
...
function C:init()
  ...
  self.differentValuesFieldFlags = {}
...
local function drawInputFloat3ColoredIndicators(windowPos, cursorPos, withStepButtons)
  ...
  if withStepButtons then
    stepButtonsWidth = stepButtonSize.x * 2 - style.ItemSpacing.x * 3
  end
  ...
  local coloredBG_X_StartPos_X = startX
  local coloredBG_X_EndPos_X = coloredBG_X_StartPos_X + coloredBGWidth

  local coloredBG_Y_StartPos_X = startX + elementWidth + style.ItemSpacing.x
  local coloredBG_Y_EndPos_X = coloredBG_Y_StartPos_X + coloredBGWidth

  local coloredBG_Z_StartPos_X = startX + elementWidth * 2 + style.ItemSpacing.x * 2
...
function C:valueEditorGui(fieldName, fieldValue, arrayIndex, fieldLabel, fieldDesc, fieldType, fieldTypeName, customData, pasteCallback, contextMenuUI, indeterminateFlags)
  floatFormat = "%0." .. editor.getPreference("ui.general.floatDigitCount") .. "f"
  local fieldNameId = "##" .. fieldName .. (arrayIndex or "")
  local isDifferent = self.differentValuesFieldFlags[fieldName] ~= nil
  ...
  elseif fieldTypeName == "TypeString"
      ...
      or fieldTypeName == "TypeI32Vector"
  ...
  -------------------------------------------------------------------------------------------------------------------------
  elseif fieldType == "int" or fieldType == "char" or fieldType == "TypeI32" or fieldType == "TypeI8" then
    if not isDifferent then
  ...
  elseif fieldType == "Point3F" or fieldType == "vec3" or fieldType == "MatrixPosition" then
    ...
    local result = editor.uiInputFloat3Ex(fieldNameId, self.input4FloatValue, nil, nil, floatFormat, indeterminateFlags, self.editEnded)
  ...
  elseif fieldType == "Point2F" then
    ...
    local result = editor.uiInputFloat2Ex(fieldNameId, self.input4FloatValue, nil, nil, floatFormat, indeterminateFlags, self.editEnded)
  ...
  elseif fieldType == "EaseF" then
    ...
    local result = editor.uiInputFloat4Ex(fieldNameId, self.input4FloatValue, nil, nil, floatFormat, indeterminateFlags, self.editEnded)
  ...
  elseif fieldType == "Point2I" then
    ...
    local result = editor.uiInputFloat2Ex(fieldNameId, self.input4FloatValue, nil, nil, integerFormat, indeterminateFlags, self.editEnded)
