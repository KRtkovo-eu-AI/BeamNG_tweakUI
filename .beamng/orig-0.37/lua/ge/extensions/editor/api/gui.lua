...
local function getDefaultIconButtonSize()
  return defaultIconButtonSize
end
...
local function uiIconImage(icon, size, col, borderCol, label)
  if not size then
    tempVec2A.x = 32 * imgui.uiscale[0]
    tempVec2A.y = 32 * imgui.uiscale[0]
  else
    tempVec2A.x = size.x * imgui.uiscale[0]
    tempVec2A.y = size.y * imgui.uiscale[0]
  end
  size = tempVec2A
...
--TODO: get docs web url root from an editor cpp api (depending on shipping_build)
local docsWebRoot = "https://documentation.beamng.com/"

local function uiHelpButton(topicName, relativeUrl, size)
  if uiIconImageButton(editor.icons.help, size or editor.miniIconButtonSize, nil, nil, nil, topicName .. "HelpButton") then
    openWebBrowser(docsWebRoot .. (relativeUrl or "index"))
  end
...
local function uiInputFile(label, buf, width, dir, extFilter, flags, editEnded)
  ...
  local res = uiInputText("##file_"..label, buf, nil, flags, nil, nil, editEnded)
  if editEnded then
    editEnded[0] = imgui.IsItemDeactivatedAfterEdit()
  end
  ...
  if fileDialogContexts[label][0] then
    res = true
    fileDialogContexts[label][0] = false
    if editEnded then
      editEnded[0] = true
    end
  end
...
local function showWindow(windowName, show)
  ...
  if not wnd then
    ...
    if show ~= nil then
      wnd.visible[0] = show
      if show then
        extensions.hook("onEditorToolWindowShow", windowName)
      else
        extensions.hook("onEditorToolWindowHide", windowName)
      end
    else
...
local editorModalDialogName = "editorModalDialog"

local function openModalDialog(title, windowSize, uiFunction)
  editor.modalDialogTitle = title
  editor.modalDialogUiFunc = uiFunction
  editor.registerModalWindow(editorModalDialogName, windowSize, nil, true)
  editor.openModalWindow(editorModalDialogName)
end

local function closeModalDialog()
  editor.modalDialogTitle = nil
  editor.modalDialogUiFunc = nil
  editor.unregisterWindow(editorModalDialogName)
end

-- called by the editor, internal
local function modalDialogWindowUi()
  if editor.modalDialogUiFunc then
    if editor.beginModalWindow(editorModalDialogName, editor.modalDialogTitle) then
      editor.modalDialogUiFunc()
    end
...
local function addGuiCoreApi()
  ...
  editor.getDefaultIconButtonSize = getDefaultIconButtonSize
...
local function addUndoReadyWidgetsApi()
  ...
  editor.uiInputFile = uiInputFile
...
local InputFloat_IndeterminateText = "---"

local function uiInputFloatEx_componentInput(v, v_index, widgetId, step_buttons, step, step_fast, format, intermediate, editEnded, componentWidth)
  -- styling
  local style = imgui.GetStyle()
  ...
  if step_buttons then
    componentWidth = componentWidth - stepButtonSize.x * 2 - style.ItemSpacing.x * 2
  end

  local val = ""
  if intermediate then
    val = InputFloat_IndeterminateText
  else
  ...
  if imgui.InputText(widgetId .. "_inputtext", editor.getTempCharPtr(val), nil, imgui.flags(imgui.InputTextFlags_CharsDecimal, imgui.InputTextFlags_AutoSelectAll)) then --, imgui.InputTextFlags_EnterReturnsTrue
    ...
    local newVal = nil
    local hasMathOperator = string.match(val, "(%d+)[+|%-|%*|%/]%d+")
    if hasMathOperator then
      newVal = eval(val)
    else
      newVal = tonumber(val)
    end
...
local function uiInputFloatEx(label, v, step, step_fast, format, flags, editEnded)
  local addStepButtons = step ~= nil or step_fast ~= nil
  if addStepButtons then
    step = step or step_fast / 2.0
    step_fast = step_fast or step * 2.0
  end
  format = format or "%.3f"
  editEnded[0] = false

  local lbl, id = string.match(label, "([%w%s]*)##([%w%s]*)")
  ...
  return { changed = valueChanged, indeterminateFlags = returnedIndeterminateFlags}
end

local function uiInputFloat2Ex(label, v, step, step_fast, format, flags, editEnded)
  local addStepButtons = step ~= nil or step_fast ~= nil
  if addStepButtons then
    step = step or step_fast / 2.0
    step_fast = step_fast or step * 2.0
...
local function uiInputFloat3Ex(label, v, step, step_fast, format, flags, editEnded)
  local addStepButtons = step ~= nil or step_fast ~= nil
  if addStepButtons then
    step = step or step_fast / 2.0
    step_fast = step_fast or step * 2.0
...
local function uiInputFloat4Ex(label, v, step, step_fast, format, flags, editEnded)
  local addStepButtons = step ~= nil or step_fast ~= nil
  if addStepButtons then
    step = step or step_fast / 2.0
    step_fast = step_fast or step * 2.0
...
local function addExWidgetsApi()
  editor.uiInputFloatEx = uiInputFloatEx
  editor.uiInputFloat2Ex = uiInputFloat2Ex
  editor.uiInputFloat3Ex = uiInputFloat3Ex
  editor.uiInputFloat4Ex = uiInputFloat4Ex
  editor.InputFloat_IndeterminateX = InputFloat_IndeterminateX
  editor.InputFloat_IndeterminateY = InputFloat_IndeterminateY
  editor.InputFloat_IndeterminateZ = InputFloat_IndeterminateZ
  editor.InputFloat_IndeterminateW = InputFloat_IndeterminateW
end

local function addOtherWidgetsApi()
  editor.uiHelpButton = uiHelpButton
  editor.openModalDialog = openModalDialog
  editor.closeModalDialog = closeModalDialog
  editor.modalDialogWindowUi = modalDialogWindowUi
end
