...
local function setCleanExitInfo()
...
local dirtyToolsSaveInfo = nil

local function saveDirtyToolsUi()
  if editor.beginModalWindow("saveDirtyTools", "Save Tool Windows Data") then
    if not dirtyToolsSaveInfo then
      dirtyToolsSaveInfo = {}
...
local function requestCollisionRebuild()
  editor.needsCollisionRebuild = true
end

local function setDirtyTool(name, saveFn)
  editor.dirtyTools[name] = saveFn
  setCleanExitInfo()
end

local function resetDirtyTool(name)
  editor.dirtyTools[name] = nil
end

local function resetAllDirtyTools()
  editor.dirtyTools = {}
...
local function selectEditMode(newEditMode)
  ...
  if oldEditMode then
    ...
    extensions.hook("onEditorEditModeDeactivated", oldEditMode)
  end
  ...
  if newEditMode then
    ...
    extensions.hook("onEditorEditModeActivated", newEditMode)
  end
...
local function muteAudio(muteIt)
  if muteIt or muteIt == nil then
    ...
    editor.currentGameAudioVolume = nil
    editor.muted = false
  end
end
...
local function initialize(editorInstance)
  ...
  editor.AuxControl_LMB_Drag = blshift(1, 18)
  ...
  editor.history.onRedo = function(action) if editor.showNotification then editor.showNotification("Redo: " .. action.name) end end
  editor.levelPath = ""
  editor.dirty = false
  editor.editModes = {}
  editor.editMode = nil
  ...
  editor.saveDirtyToolsUi = saveDirtyToolsUi
  editor.requestCollisionRebuild = requestCollisionRebuild
  editor.setDirty = setDirty
  editor.resetDirty = resetDirty
  editor.setDirtyTool = setDirtyTool
