    ...
    if objectIdByIconClick and objectIdByIconClick ~= 0 then
      ...
      if not doingRectSelection
        ...
        if rayCastInfo then
          ...
          local foundObject = hoveredObject
          repeat
            foundObject = Engine.Prefab.findPrefabForChild(foundObject)
            if foundObject then
              prefab = foundObject
            end
          ...
          if prefab then
            -- hoveredObject is the actual object in the scene. If the prefab is already selected, then leaving hoveredObject alone will allow the user to select a child of the prefab
