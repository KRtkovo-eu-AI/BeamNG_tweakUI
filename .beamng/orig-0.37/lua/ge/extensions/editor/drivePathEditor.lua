...
local function handleMainToolWindowUI()
  if editor.beginWindow(toolWinName, "Drive Path Editor###8874", im.WindowFlags_NoCollapse) then
    ...
    if selSpline and selSpline.isFreeMode and #drivePathSplines > 0 and not isPlaying and not isRecording then
...
local function onEditorGui()
  ...
  splineMgr.updateDirtyDrivePathSplines(isBarsLimit)
  ...
  if not selSpline or selSpline.isFreeMode then -- ** FREE MODE FRONT END: **
    ...
    input.handleSplineEvents(
      splines,
      out,
      false, true, true, true, isBarsLimit, false, true, isLockShape,
      defaultSplineWidth,
    ...
    -- Handle the mouse and keyboard events.
    input.handleNavGraphEvents(selSpline, graphData.nodes, splineMgr.deepCopyDrivePathSpline, splineMgr.singleSplineEditUndo, splineMgr.singleSplineEditRedo, isBarsLimit)
    ...
    render.renderVelocitiesGraph(selSpline, velocityUnitsInt, elevScale, isBarsLimit)
  end
end
