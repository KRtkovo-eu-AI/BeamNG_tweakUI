...
local apack = {
	...
	'/lua/ge/extensions/editor/gen/test',
...
--		if mode == 'conf' then return end
--	unrequire(apack[8])
--	rerequire(apack[8])
--		print('??^^^^^^^^^^^^^^^^^ if_test:'..tostring(Tst.onUp))
	if mode then
		U._MODE = mode
		U.out._MODE = mode
	end
...
if U._PRD == 1 and not U._DEMO then stage = nil end
...
if U._DEMO then
	U.camSet({-12.61, 7.10, 7.75, 0.0198367, 0.0429585, -0.906865, 0.418757})
end
...
--			local fo = createObject(fprod and 'ForestItemData' or 'TSForestItemData')
			if pnew ~= nil then
				fo:setField('shapeFile', 0, pnew)
			else
				fo:setField('shapeFile', 0, pth)
			end
--			if fprod then fprod.close() end
			...
			ddae[pth].fo = fo
			-- tolevel
			local nm = U.shape2name(pth)
			W.adae[nm] = {
				name = nm,
				internalName = nm..'_int',
				class = 'ForestItemData',
--				class = fprod and 'ForestItemData' or 'TSForestItemData',
				persistentId = 'ee1216e3-ff66-4ee3-b7f8-8166acc8383b',
				annotation = 'OBSTACLES', -- 'MODULES',
				shapeFile = pth
			}
		end
	end
...
local cij
local cedit = {mesh = nil, part = nil, forest = nil, aforest={}, cval = {}} -- (id, id, item_key, starting_params_vals)
...
--		lo("?? ccc:"..tostring(W.ui.gen_center))
	L = ext.x
	out.L = L
	W.ui.build_radius = L/2
	local apos = {}
...
--				lo('?? FOR_ALL:'..tostring(desc.selection))
...
W.forSpline = function()
	local desc = adesc[cedit.mesh]
--		lo('?? forSpline:'..tostring(desc.selection))
...
local function toJSON(desc, pth)
		lo('>>^^^^^^^^^^^^^^^^ toJSON:'..#asave)
--    if #asave > 5 then
...
W.toLevel = function(desc, pth)
	...
	-- shape
	local batdir = pth or editor.getLevelPath()..'bat/'
	local nm = 'b_'..(desc.idpersist or desc.id)
	if not desc.idpersist then
		desc.idpersist = desc.id
	end
	M.toDAE(desc.data, batdir..nm..'.dae')
	-- forest
--	local dpart = {}
	local apart = {}
	local ss = scope
...
--			lo('?? for_wall:'..tostring(ij[2]))
	...
	--                    U.dump(trans,'?? for_fi:'..tostring(item)..':'..tostring(pos)..':'..tostring(scale))
				...
				-- to level
				local nm = U.shape2name(pth)
--				if not dpart[nm] then dpart[nm] = {} end
--								U.daump(trans, '?? for_pos:'..tostring(pos)..':'..tostring(scale))
--								U.dump(rot, '?? for_pos:'..tostring(pos)..':'..tostring(scale))
--					lo('??******* for_pos:'..tostring(pos)..':'..tostring(scale)..':'..tostring(rot)..'<>'..tostring(trans)..':'..tostring(trans:getColumn(3)))
				local c1,c2,c3 = trans:getColumn(0),trans:getColumn(1),trans:getColumn(2)
				apart[#apart +1] = {
--				dpart[nm][#dpart[nm]+1] = {
					ctxid = desc.id,
					pos = {pos.x, pos.y, pos.z},
					rotationMatrix = {c1.x,c1.y,c1.z,c2.x,c2.y,c2.z,c3.x,c3.y,c3.z},
					scale = scale,
					type = nm
				}
			end
...
--	fout = io.open(fname, 'w')
	...
	fout:close()
--[[
	for nm,v in pairs(dpart) do
		local fname = fdir..nm..'.forest4.json'
		local fout = io.open(fname, 'r')
			lo('?? if FILE:'..fname..':'..tostring(fout))
		if not fout then
			-- create file
			fout = io.open(fname, 'w')
			fout:close()
			fout = io.open(fname, 'r')
		end
		local data = fout:read('*all')
		fout:close()
		local jdata = jsonEncode(v)
		jdata = jdata:gsub('},{', '}\r\n{')
		fout = io.open(fname, 'w')
			lo('?? for_file:'..fname..':'..tostring(#data)..':'..#v..':'..jdata)
		fout:write(string.sub(jdata, 2, #jdata-1))
...
W.houseSave = function(d, nm)
--	local nm = 'b_'..d.id
--		lo('??+++ for_DESC:'..nm..':'..tostring(d.data and #d.data or nil))
	local batdir = editor.getLevelPath()..'bat/'
	toJSON(d, batdir..nm..'.json')
...
W.forJSON = function(obj)
	if not obj then return end
	local idstat = obj:getID()
		lo('?? forJSON:'..tostring(obj.shapeName)..' STAT:'..idstat..':'..tostring(cedit.mesh))
	local nm = obj.name
	if not nm then
		nm = U.shape2name(obj.obj.shapeName)
	end
		lo('?? forJSON_nm:'..tostring(nm))
	if cedit.mesh then
		-- save edited
		local om = scenetree.findObjectById(adesc[cedit.mesh].idstatic)
			lo('?? proc2stat:'..cedit.mesh..':'..tostring(nm)..':'..adesc[cedit.mesh].idstatic)
		W.houseSave(adesc[cedit.mesh], U.shape2name(om.obj.shapeName))
--		W.houseSave(adesc[cedit.mesh], obj.name)
		om.hidden = false
--		W.houseDown(adesc[cedit.mesh])
--		return
	end
--		if true then return end
	obj.hidden = true
	local asplit = U.split(obj.shapeName, '.', true)
	local id = fromJSON(asplit[1]..'.json')
	-- start edit
	local desc = W.houseUp(nil, id)
	desc.idpersist = tonumber(U.split(nm, '_', true)[2])
	desc.idstatic =  idstat
		lo('??_________________ fromStatic:'..desc.idstatic..'/'..desc.idpersist)
	W.markUp()
	cij = W.forHit(getCameraMouseRay(), adesc[desc.id])
	W.scopeOn('building')
	if W.out.adf and W.out.adf[desc.idpersist] then
...
--                    cforest = {dae, i}
	...
	--					lo('?? for_fitem:'..tostring(pos)..':'..#its)
					for i,o in pairs(its) do
						local key = o:getKey()
						if not dhide[key] then
							if not dforest[key] then
								-- static forest
								dhide[key] = o
	--								lo('?? to_hide:'..key)
							end
...
--	cij = W.forHit(getCameraMouseRay(), adesc[cedit.mesh])
--[[
		if false then
--			local id = 0
...
--								U.dump(rot, '?? for_pos:'..tostring(pos)..':'..tostring(scale))
							...
							local om = createObject('TSStatic')
							om:setField('shapeName', 0, pth)
							om.isMesh = true
							om.canSave = false
							om:registerObject('dae_'..id)
							groupBat:add(om.obj)
							om:setTransform(trans)
						end
					end
			...
			W.houseDown(desc)
			scenetree.findObjectById(desc.idstatic).hidden = false
			lo('?? if_stat:'..tostring(desc.idstatic))
		end
]]
		U.dump(cij, '<< forJSON:')
end
...
--        lo('?? if_forest3:'..tostring(forest)..':'..tostring(forestName),true)
		if forestName ~= 'theForest' then
			...
			if forest then
				forestName = 'theForest'
--				lo('?? if_forest:'..tostring(forest:getData()))
			else
...
--        	lo('?? edit_CREADTED:'..tostring(scenetree.findObject('edit'))) --oedit))
    ...
    if not groupBat then
        ...
        if scenetree.MissionGroup then
            scenetree.MissionGroup:addObject(groupBat)
        end
        groupBat = scenetree.findObject('bat')
...
--						lo('?? for_OBJ:'..tostring(om)..':'..tostring(file)..':'..dirname..astep[#astep])
					if file then
	...
	--				lo('?? for_b:'..tostring(o)..':'..tonumber(o)..':'..tostring(om.obj.shapeName)) --..':'..tostring(obj.obj:getField('shapeName')))
			end
		end
	end
...
--		U.camSet({-1.74, 6.41, 190.93, -0.136776, -0.253552, 0.842797, -0.45464})
	end
--            if true then return end
...
--		lo('?? if_FOREST:'..#core_forest.getForestObject():getData():getItems())
--	  	lo('>>____++++_____ up:'..tostring(scope)..':'..tostring(forest.name)..' fdata:'..tostring(fdata)..':'..#fdata:getItems()) --..':'..tostring(editor.getPreference("roadEditor.general.aiRoadsSelectable")))
...
--                U.dump(daePath['pillar'], '?? DDAE:')
			local file = io.open(ok..'managedItemData.json', 'r')
			local cnt = {}
			if file then
				file:close()
...
---			local file = io.open(ok..'managedItemData.json', 'w')
--			file:close()
--				U.dump(W.adae, '??______________ ADAE:')
--			jsonWriteFile(ok..'managedItemData.json', W.adae)
--			if U._PRD ~= 0 then
--			end
		else
...
--        console.clearConsole() -- not working
	...
	out.dyell = nil
--	out.ccommand = ''
--		lo('?? clear:'..tostring(mode)..':'..tostring(U._MODE), true) --..#editor.getAllRoads())
	if groupBat then
		local list = groupBat:getObjects()
...
--			lo('??_____________________ clear.for_items: adesc:'..tableSize(adesc)) --..':'..#fdata:getItems())
	...
	if mode ~= 'conf' and forest then
--			lo('??^^^^^^^^^^^^^^^^^^^^^^^^^ clean_forest:') --..tostring(forest)..':'..tostring(fdata))
		fdata = forest:getData()
		if fdata then
			local list = fdata:getItems()
--				lo('??_____________________ f_stat:'..tostring(#list))
			for _,f in pairs(list) do
				editor.removeForestItem(fdata, f)
			end
				lo('?? fclean_post:'..#fdata:getItems())
		end
	end
...
--        lo('?? TO:'..tostring(fobj)..':'..tostring(fdata)..':'..tostring(cforest), true)
...
W.splineUp = function(desc, ifloor, key, sdata) -- cbase)
	if not desc then return end
	local inupdate = sdata.id
	local H = forHeight(desc.afloor, ifloor-1)
--[[
		sdata.base = {
			vec3(-17.44395256,-4.709114075,0),
			vec3(-23.22937523,-4.709114075,0),
			vec3(-24.22937523,0.290885925,0),
			vec3(-29.44395256,0.290885925,0),
		}
]]
--		U.dump(sdata,'>> splineUp:'..key..':'..ifloor..':'..tostring(H)..':'..tostring(sdata))
		lo('>> splineUp:'..tostring(inupdate)..':'..key..':'..ifloor..':'..tostring(H)..':'..tostring(sdata)..':'..tostring(scope))
	local aw = U.split(key, '_')
--	table.remove(aw, 1)
--		U.dump(aw, '?? AW:')
	-- modules params
	local winleft,winbot,win,door =
		inupdate and sdata.winleft or 0.2,
		inupdate and sdata.winbot or 1
	local adoorind = sdata.adoorind or {}
	sdata.winleft = winleft
	sdata.winbot = winbot
	local h = desc.afloor[ifloor].h
--	local adae = {}
	-- selection base
	local base
	if inupdate then
...
--				lo('?? if_DOOR:'..tostring(door)..':'..tostring(wall.doorind))
			if not sdata.adoorind and #adoorind == 0 and wall.doorind then
	--			door = ddae[wall.door]
--				doorind = wall.doorind
				adoorind[#adoorind+1] = wall.doorind
	--			adae['door'] = wall.door
				sdata.def['door'] = {dae=wall.door}
--				break
			end
		--						break
		end
		local wlast = desc.afloor[ifloor].awall[aw[#aw]]
		base[#base+1] = wlast.pos + wlast.u
		for i=1,#base do
			base[i].z = 0
		end
	end
--		U.dump(adoorind, '?? for_di:'..#adoorind)
	sdata.adoorind = adoorind
--[[
		base = {
			vec3(-17.44395256,-4.709114075,0),
			vec3(-23.22937523,-4.709114075,0),
	...
	win = sdata.def['win'] and ddae[sdata.def['win'].dae] or nil
	door = sdata.def['door'] and ddae[sdata.def['door'].dae] or nil
	-- last
--	if inupdate then base = sdata.base end
--		U.dump(base, '?? for_BASE:')
--		U.dump(win, '?? for_WIN:'..#sdata.aforest)
	-- spline base
	local ap,L,adist,astep = U.path2spline(base)
--		U.dump(astep, '?? ASTEP:')
	-- make holes
	local aforest
	local ahole = {}
...
--			local ar,step = {},winleft
		local d,i = winleft
--			lo('??____*****____ sU:'..tostring(sdata.default)..':'..tostring(sdata.spacefit)..':'..#aforest..' d:'..d)
--		local spacetot,d,i = 0,winleft
		local i = 0
		local saforest = deepcopy(aforest)
		if sdata.default then
			-- recalc winleft to center
			while i < 200 do
				i = i + 1
				local ds,w
--					lo('?? spline_FOR_i:'..i..'/'..#sdata.aforest..':'..d)
				if i <= #sdata.aforest then
					if sdata.spacefit then
						aforest[i].p.x = d
					else
						if aforest[i].p.x > d then
							d = aforest[i].p.x -- + ddae[aforest[i].dae].w + 0.5
						end
					end
					w = ddae[aforest[i].dae].w
--						lo('??+++++ for_D1:'..i..' d:'..tostring(d)..' d+w:'..(d+w)..' L:'..L)
--						U.dump(aforest[i],'??+++++ for_D1:'..i..':'..tostring(d)..' d+w:'..(d+w)..' L:'..L)
					if L - (d + w) > winleft - U.small_val then
						if sdata.spacefit then
							d = d + w + padding
						end
--						d = d + winspace
					else
--						lo('?? break1:'..i..' d:'..d..' w:'..w..'/'..L)
						break
					end
					--TODO:
--					ds = aforest[i].space -- ddae[aforest[i].dae].w
--					d =
				else
					if #sdata.aforest>0 and i == #sdata.aforest+1 then
						d = d + (w or win.w) + padding
--							lo('?? D2+1:'..#sdata.aforest)
					end
					-- default
					local w,space,tp,bot = win.w,winspace,'win',winbot
--						lo('?? to_FOR:'..i..':'..(d+w)..'/'..L..':'..space)
					if #U.index(adoorind,i)>0 then
						w,space,tp,bot = door.w,doorspace,'door',0
					end
--						lo('??+++++ for_D2:'..i..' d:'..d..' w:'..w..' d+w:'..(d+w)..' L:'..L..' wl:'..winleft..':'..sdata.winleft)
					if L - (d + w) > winleft - U.small_val then
--						ds = (win.w + 0.5)
...
--							lo('?? D2+2:')
						d = d + space
--[[
							U.dump(aforest,'?? afor+:'..d..':'..#saforest)
							if inupdate then
								_dbdrag = true
							end
]]
					else
--						lo('?? break2:'..i..' saf:'..#saforest)
						break
					end
--					d = d + win.w + 0.5
				end
--				if L-(d + ds) < winleft - U.small_val then break end
--				spacetot = spacetot + ds
			end
--			lo('?? space_TOT:'..i..':'..spacetot..'/'..L..' WL:'..winleft..':'..win.w)
--			if inupdate then
			if #saforest>0 and inupdate and not sdata.spacefit then
--				aforest = saforest
				for ind,_ in pairs(aforest) do
					if ind<=#saforest then
						aforest[ind] = saforest[ind]
--					else
--						lo('??++++++++++++++++++++++++++++++ ind_MORE:'..ind)
--						break
					end
				...
				local swl = winleft
				-- update winleft
				winleft = (L - (d-sdata.def['win'].padding) + winleft)/2
--				sdata.winleft = winleft
--					lo('?? WL:'..winleft..' swl:'..swl)
				local dwl = winleft-swl
				-- update forest positions
				for _,d in pairs(aforest) do
					d.p.x = d.p.x + dwl
				end
			end
--			winleft = (L - spacetot)/2
--			d = winleft
		elseif inupdate then
			aforest = saforest
		end
--			U.dump(aforest,'??^^^^^^^^^^^^^^^^^^^ for_AF:'..tostring(inupdate)..':'..tostring(sdata.default)..':'..#aforest)
--			i = 0
--			if inupdate then return end
--			aforest[3].p = aforest[3].p + 1*vec3(1,0.2,0)
--			aforest = {aforest[1],aforest[2],aforest[3],aforest[4],aforest[5]}
			lo('?? for_AFOR:'..#aforest)
...
--[[
		while L - d >= winleft - U.small_val and i < 200 do
			i = i + 1
--		for i = 1,nwin+2 do
			if i == doorind then
				...
				aforest[#aforest+1] = {dae = sdata.def['door'].dae, type='door', shift=nil}
				d = d + door.w + (doorspace - door.w)/2
			end
			d = d + (winspace - win.w)/2
			ahole[#ahole+1] = {p=vec3(d,winbot), w=win.w, list={
				vec3(0,0),vec3(win.w,0),vec3(win.w,win.h),vec3(0,win.h)
			}}
			aforest[#aforest+1] = {dae = sdata.def['win'].dae, type='win', shift=nil}
			d = d + win.w + (winspace - win.w)/2
			if L - d < winleft then
--					lo('??__________ break:')
				break
			end
	...
	sdata.spacefit = nil
--		U.dump(aforest, '?? for_AF0:')
	sdata.ap = ap
	sdata.adist = adist
	if not sdata.ahole then sdata.ahole = {} end
	for i,h in pairs(sdata.ahole) do
		ahole[#ahole+1] = h
		ahole[#ahole].skip = true
	end
	sdata.ahole = {}
		table.sort(ahole, function(a,b)
			return a.p.x<b.p.x
		end)
--		table.remove(ahole,6)
--		U.dump(ap, '?? pre_spline:'..tostring(ap)..':'..#adist..':'..#astep..':'..h..':'..L)
--		if true then return end
--		U.dump(ahole, '?? for_AHOLE:'..#ahole..':'..#aforest)
--[[
		lo('??________________________ pre_spline:'..tostring(inupdate)..':'..#ahole..':'..tostring(#ap)..':'..#adist..':'..#astep..':'..h..':'..L..':'..#aforest)
		U.dump(ahole, '?? for_AHOLE:')
		U.dump(ap, '?? for_AP:')
		U.dump(adist, '?? for_ADIST:')
		U.dump(astep, '?? for_ASTEP:')
		U.toFile({
			base={vec3(0,h),vec3(L,h),vec3(L,0),vec3(0,0)},
			ahole=ahole,
			ap=ap,
			adist=adist,
			astep=astep
...
--		lo('?? for_body:'..#mbody.verts)
--		U.dump(afpos, '?? splineUp_afpos:')
	if not mbody then return end
	local j = 0
	for i=#ahole,1,-1 do
		if ahole[i].skip then
--				U.dump(ahole[i], '?? for_DAE:'..tostring(scale))
			table.insert(aforest,i,{
				scale = {},
				list = ahole[i].list,
				dae=ahole[i].dae,type='win',p=ahole[i].p}) --list[1]})
		end
	end
	-- set sacle
	local margin = 0.06
	for i,d in pairs(aforest) do
		if d.scale then
			...
			local scale = vec3(
				...
				(ahole[i].list[#ahole[i].list].y-ahole[i].list[1].y)/(dae.to.z-dae.fr.z-margin))
			d.scale = scale
		end
	end
...
--		if not h.skip then
--			j = j + 1
--	for i,_ in pairs(aforest) do
	for i,d in pairs(afpos) do
--			local d = afpos[i]
		aforest[i].pos = (d[1]+d[2])/2 + vec3(0,0,d[3].y)
--[[
		if aforest[i].list then
			aforest[i].pos = (aforest[i].list[1]+aforest[i].list[2])/2 + vec3(0,0,d[3].y)
		else
			aforest[i].pos = (d[1]+d[2])/2 + vec3(0,0,d[3].y)
		end
]]
--		end
	...
	sdata.aforest = aforest
--		U.dump(aforest, '?? for_AF:')
	for i=1,#mbody.verts do
		mbody.verts[i].z = mbody.verts[i].z + H
	end
	mbody.material = sdata.mat or desc.afloor[ifloor].awall[aw[1]].mat
--			U.dump(afpos,'??^^^^^^^^^^^^^^^^^^^ for_AF:'..tostring(inupdate)..':'..tostring(sdata.default)..':'..#aforest)
	return mbody,afpos,base,aforest,invalid
end
...
--            lo('?? for_SF:'..desc.u:length())
		elseif desc.win and not desc.isbox then
			local margin = 0.06
...
--					lo('?? wallUp.nw:'..desc.u:length()..' aval:'..aval..' wsapce:'..winspace..' start:'..start)
--??				local aywin = ay
...
--					hasdoor = nil
				local wi = 0
				local pilshift = 0
				for p = start,start + aval - winspace - small_dist,winspace do
					wi = wi + 1
--						lo('?? for_wp:'..p..'/'..desc.u:length()..':'..(start + aval - winspace))
...
--              end
--						lo('?? wallUp_wpos:'..winbotc..':'..dz..':'..winbot)
 					local itemkey = to(
						desc, dae,-- desc.win,
...
--			U.dump(desc.selection, '??+++++++++++++++++ if_SELECTION:')
		local afspline = {}
		forBuilding(adesc[id], function(w, ij)
--				lo('?? mU:'..ij[1]..':'..ij[2])
--                    lo('?? mark:'..ij[1]..':'..ij[2], true)
	--            lo('?? to_mark:'..ij[1]..':'..ij[2]..':'..tostring(forestType)..':'..tostring(forestMeshName))
--				lo('?? for_IJ:'..tostring(ij[2])..':'..tostring(isnan(ij[2])))
			if not tonumber(ij[2]) then
				local H = forHeight(desc.afloor,ij[1]-1)
--					lo('?? for_SPLINE:'..ij[1]..':'..ij[2]..':'..H)
				if forestMeshName then
					local s = desc.aspline[ij[1]][ij[2]]
--						U.dump(s.aforest, '??^^^^ SPLINE_FOREST:'..forestMeshName..':'..ij[2]..':'..tostring(forestType))
					local H = forHeight(desc.afloor,ij[1]-1)
					for _,d in pairs(s.aforest) do
						if d.dae == forestMeshName then
--								U.dump(s.aforest, '??^^^^^^^^^^^^^^^^^^^^ AFSPLINE: key:'..tostring(cedit.forest)..' scope:'..tostring(cedit.fscope))
...
--						local u = dfcorner[ij[1]] and or w.u
						...
						if desc.aspline and desc.aspline[ij[1]] then
							for key,_ in pairs(desc.aspline[ij[1]]) do
								local aw = U.split(key,'_')
--									U.dump(aw, '?? spln_BASE:')
								if #U.index(aw, ij[2]) > 0 then
									inspline = true
									break
								end
							end
						end
						if w.pos and not inspline then
							local pth = {}
...
--                        lo('?? mark_CORNER:')
		...
		end, nil, true) --, nil, nil, true)
--[[
		if #afspline > 0 then
				lo('?? for_AFSPLINE:'..#afspline)
...
--        U.dump(out.dyell, '?? mU_dyell:')
	if #cedit.aforest>0 then
		...
		out.avedit = {}
		if s then
--				U.dump(s.aforest, '?? mU_forest:'..#cedit.aforest..':'..cij[1])
--				U.dump(cedit.aforest, '?? mU_forest:'..#cedit.aforest..':'..cij[1])
...
--[[
		...
		for _,d in pairs(s.aforest) do
			if d.dae == forestMeshName then
--								U.dump(s.aforest, '??^^^^^^^^^^^^^^^^^^^^ AFSPLINE: key:'..tostring(cedit.forest)..' scope:'..tostring(cedit.fscope))
				if cedit.fscope ~= 1 or cedit.forest==d.key then
					out.avedit[#out.avedit + 1] = d.pos+vec3(0,0,H) -- dforest[d.key].item:getPosition()
				end
			end
		end
]]
	elseif forestMeshName == nil and cij then
		if cij[3] then
			out.dyell = {}
...
W.selectionTurn = function(ang, center)
...
--                return
	...
	for i,f in pairs(desc.afloor) do
		forestClean(f.top)
		local cheight = forHeight(desc.afloor,i-1)
		f.ij = {i}
...
--							lo('?? sPLINED:'..k..':'..tostring(id))
					...
					s.box = s.o:getWorldBox()
					s.base = base
					s.aforest = aforest
					-- place forest
					local wall = f.awall[1]
					--TODO:
	--					lo('?? DZ:'..dz)
...
--							U.dump(iforest,'??*****____________ if_IFOREST:'..#cedit.aforest..':'..preaf..':'..preceaf)
						if #iforest>0 and preceaf>0 then
							cedit.aforest = iforest
							cedit.forest = nil
						elseif preaf==0 and #s.aforest>0 then
							cedit.aforest = U.map(aforest,function(k,v)
								return v.key
							end)
--??						elseif false and not cedit.forest then
--							cedit.forest = aforest[1].key
						end
					end
				else
...
--[[
					...
					if w.splined then
						-- restore mesh
						am[1].material = w.splined.mat or am[1].material
						local cam = {}
						for k = 1,spany do
							table.insert(cam, 1, am[1])
						end
...
W.houseDown = function(desc, fkeep)
	if desc.prn then
...
--			forestClean(f.top)
		...
		if not W.out.adf then W.out.adf = {} end
		W.out.adf[desc.idpersist] = adf
	end
...
-- generate procedural buuilding in the area
...
--			lo('?? for_file:'..i..':'..s)
	...
	local ashape = {'l_shape',
		--'sh_t',
		'sh_u', 'sh_square', 'sh_cross', 'l_shape'}
--		L = 128
--		tb = extensions.editor_terrainEditor.getTerrainBlock()
	for i=1,num do
		local base = shape2base(ashape[math.random(#ashape)])
		local p
		if istemplate then
			p = vec3(0,0,0)
		else
...
--[[
	...
	local winspace, winbot, winleft = 1.6 + 0*math.random(), 1, 0.2
...
--        rshape = ingen and ({'shed','gable'})[math.random(1,2)] or 'flat'
	...
	if intest then
		mat = dmat.roof[math.random(1, #dmat.roof)]
	elseif rshape == 'flat' then
...
--		U.dump(afloor[1].awall[1], '?? for_MAT:')
	-- render
	return houseUp({pos = p, afloor = afloor, basement = {yes = false}, aspline={}})
...
--			lo('?? cb_JOB:')
		local pos,w,h
		forBuilding(desc, function(wall,ij)
			local u = wall.u
			w = u:length()
			h = wall.v:length()
...
W.buildingLOD = function(desc, arc)
	local dlod_name = 'exp_lod'
	local texture_name = 'm_shot5'
	local dlod
...
--			local lodfn = dirname..dlod_name..'_1.dae'
		local id = 1
	dlod.name = dirname..dlod_name..'_'..id..'.dae'
	W.persist({dlod})
	M.toLOD(dlod, editor.getLevelPath()..'bat/'..dlod_name..'_'..id..'.dae', arc, T)
	-- build DAE file
	desc.name = dirname..'exp_main'..'_'..id..'.dae'
	W.persist({desc})
	--- import LOD geometry
	---- parse main mesh
	local xmain = M.xmlOn(desc.name)
	---- parse lod
	local xlod = M.xmlOn(dlod.name)
	---- geometries
...
--				lo('?? for_NDL:'..tostring(nd.type)..':'..tostring(nd.attr['id'])..':'..tostring(geomain.attr['id']))
	...
	local ndbody = nd
	----- top geo
	nd = M.forNode(xlod, {'COLLADA','library_geometries','geometry'}, 1, 2)
	ndmain.kids[#ndmain.kids+1] = nd

	--- top material
	local idmat = 'top-material'
	local ndmatlib = M.forNode(xmain, {'COLLADA','library_materials'})
	...
	---- scenes
	-- to scene
	local nodevs = M.forNode(xmain, {'COLLADA','library_visual_scenes','visual_scene'})
	...
	if true then
		nodevs.kids = {}
		local kid = M.toNode(nodevs, 'visual_scene', {id='Scene', name='Scene'})
--				lo('?? if_SCENE:'..kid.name)
		kid = M.toNode(kid, 'node', {id='base00', name='base00', type='NODE'})
		kid = M.toNode(kid, 'node', {id='start01', name='start01', type='NODE'})
		kid.kids = {} --{nd_maingeo}
		-- append LOD
		kid.kids[#kid.kids+1] = M.forLOD(nd_maingeo, 200, ndbody)
		local ndlod = kid.kids[#kid.kids]
		...
		ndlod.kids[#ndlod.kids+1] = ndinstg
		local ndtc = M.forNode(ndinstg, {'bind_material','technique_common'})
		ndtc.kids[#ndtc.kids+1] = ndinst
		-- append main
		kid.kids[#kid.kids+1] = M.forLOD(nd_maingeo, 1500, geomain)
	end
	---- material
	local nda = M.forNode(xmain, {'COLLADA','library_materials','material'})
	M.ofNode(nda, {name=texture_name})

	-- save DAE
	M.xml2file(xmain, desc.name)
	-- cleanup
...
--			os.execute('magick')
	...
	if true then
		...
		obj.hidden = false
		local p = '/art/shapes/common/building_architect_modules/windows/R_HS_A_WN_01_120x130.dae'
		local am = M.daeFrom(p)
		if M.valid(am) then
			local id,om = meshUp(am,'tst', groupEdit)
			om:setPosition(vec3(0,0,2))
		end
--[[
		local main = M.xmlOn(p)
			lo('??^^^^^^^^^^ for_DAE:'..p..':'..tostring(obj)..':'..tostring(main)..':'..tostring(Tst))
		local ndgeo = M.forNode(main, {'COLLADA','library_geometries'})
--						lo('?? for_GEO:'..#ndgeo.kids)
		for _,k in pairs(ndgeo.kids) do
			lo('?? for_KID:'.._..':'..k.name)
			if k.name == 'geometry' then
				local m = M.geoFrom(k)
				if M.valid({m}) then
					local id,om = meshUp({m},'tst', groupEdit)
					om:setPosition(vec3(0,0,2))
				end
					break
			end
...
-- ERR: WRONG_LOOP
	...
	if false then
		local obj = U.fromFile('/tmp/pave.json')
		local base = obj.base
--			lo('?? base:'..#base)
		local ahole = obj.ahole
		local ap = obj.ap
		local adist = obj.adist
		local astep = obj.astep
			U.dump(astep, '?? for_AS:')

		local mbody,afpos = M.frameSpline(base,ahole,ap,adist,astep)
	...
	if true then
		...
		for i,desc in pairs(adesc) do
			desc.selection = {}
			desc.selection[1] = {1,2}
--			desc.selection[1] = {1,3,2}
--			desc.selection[2] = {3,2} --{1,3,2}
--			desc.selection[2] = {1,6} --{1,3,2}
			for j,aw in pairs(desc.selection) do
				if not desc.aspline then desc.aspline = {} end --{aij={}} end
				if not desc.aspline[j] then desc.aspline[j] = {} end
				stamp = '_'..U.stamp(aw)..'_'
				desc.aspline[j][stamp] = {def={},aforest={}}
			end
				U.dump(desc.aspline, '?? d_ASPLINE:'..tostring(desc.id))
				-- start edit
			houseUp(nil, desc.id)
--			desc.selection[1] = {stamp}
--			cij = {1,stamp}
			markUp()
--				houseUp(desc, desc.id, true)
			break
		end
...
--								U.dump(ddae[wall.win], '?? for_WDAE:'..wall.win)
						...
						if not door and wall.doorind then
							door = ddae[wall.door]
							doorind = wall.doorind
						end
--						break
					...
					-- last
					local wlast = d.afloor[j].awall[aw[#aw]]
					base[#base+1] = wlast.pos + wlast.u
					if not isvalid then
						base = {}
						break
...
--			doorind = 0
		...
		local ahole = {}
		if win then
--			local winspace,doorspace = 0.5,door and 0.5 or 0
			local winspace,doorspace = win.w + 0.5,door and door.w + 0.5 or 0
			local l = L - (2*winleft+doorspace)
			local nwin = math.floor(l/winspace)
--				lo('?? for_PRE:'..L..':'..winspace*nwin..':'..(winspace*nwin + doorspace))
			winleft = (L - (winspace*nwin + doorspace))/2
...
--					U.out.aset[1] = {set=ac, c={0,1,1}, w=0.02}
--					if true then return end
		end
...
--<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		...
		local cma,adir = U.polySpline(base)
		local step = 0.2/cma
		local s2 = 2*vec3(0,0.8)+2*vec3(0.7,0)
		local mbody,pmi,pma = M.framePave({
			{vec3(0, 8), vec3(w,9), vec3(w,0), vec3(0,0)},
...
--				{vec3(6,3), vec3(8.5,3), vec3(8.5,8), vec3(6, 8)},
		...
		-- build HMAP
		--- para spline of nodes positions
		local apar = U.paraSpline(base)
			lo('?? apar:'..#apar..':'..#apin)
		local csind = 1
		local csstep,psstep = apin[csind],0
		for i,v in pairs(av) do
			if v.d > csstep then
				csind = csind+1
				psstep = csstep
			...
			local s = (v.d-psstep)/(csstep-psstep)
				lo('?? for_v:'..v.ind..'/'..csind..' d:'..v.d..'/'..psstep..' s:'..s)
			local s1 = csind == 1 and apar[csind] or apar[csind-1]
			local s2 = csind == #apin and apar[csind-1] or apar[csind]
			local a = s1[1]*s^2 + s1[2]*s + s1[3]
			local b = s2[1]*s^2 + s2[2]*s + s2[3]
			local p = U.spline2(a,b,s)
...
--			lo('?? for_ANG:'..#adir..':'..tostring(cma))
			U.dump(adir, '?? for_ANG:'..#adir..':'..cma..':'..step)
		local dir = vec3(0,-1)
		local u = U.perp(dir)
		...
		step = w/math.floor(w/step+0.5)
			lo('?? for_STEP:'..step)
				--lo('?? dist:'..i..':'..u:dot(v-pmi)..':'..tostring(v))
		-- spline
--			U.dump(mbody.verts,'?? for_U:'..tostring(u)..':'..tostring(pmi))
		local adist = {}
		local csstep = 0
		for i,v in pairs(av) do
				--lo('?? dist:'..i..':'..u:dot(v-pmi)..':'..tostring(v))
			-- spline
			local d = u:dot(v-pmi)
			local k,s = math.modf(d/step),math.fmod(d,step)/step
				lo('?? k_s:'..d..'>'..k..':'..s) --.':'..math.fmod(d,1)..':'..d/step)
--			adist[#adist+1] = u:dot(v-pmi)
		end
			U.dump(adist, '?? ADIST:'..step)
		if M.valid({mbody}) then
			meshUp({mbody}, 'tst', groupEdit)
		end
]]
...
--					psstep = csstep
				while s.p.x > csstep and istep<#ap and n<1000 do
...
--			U.out.f:write('start:')
		local mbody,pmi,pma = M.framePave(aloop, vec3(0,-1), astep)
--			U.out.f:close()
--			U.out.f = nil
--			if true then return end
...
--			if true then return end
		...
		for i,v in pairs(av) do
			while v.d > adist[csind] and N<100000 do
					N = N + 1
				csind = csind+1
			end
			local s = (v.d-adist[csind-1])/(adist[csind]-adist[csind-1])
...
--[[
		...
		for i=2,#base do
			ap[#ap+1] = base[i-1]
			--- get steps
			local cspline = aspline[i-1]
--				lo('?? for_b_i:'..i)
			if i>2 and i<#base then
				cspline = aspline[cspline.r < aspline[i-2].r and i-2 or i-1]
			end
			if i == #base then
				cspline = aspline[i-2]
			end
...
--			astep[#astep+1] = step
			...
			for j=1,na-1 do
				local p = U.line2line(
					cspline.c, cspline.c+U.vturn(base[i-1]-cspline.c,da*j),
					base[i-1],base[i])
				local s1,s2 = aspline[i-2],aspline[i-1]
				if i==2 then s1 = s2 end
				if i==#base then s2 = s1 end
				p = U.splineC(p, s1, s2, j/na)
				ap[#ap+1] = p
			end
		end
		ap[#ap+1] = base[#base]
		...
		local L = 0
		for i=2,#ap do
			astep[#astep+1] = ap[i]:distance(ap[i-1])
			L = L + astep[#astep]
			adist[#adist+1] = L
		end
]]
--		local apin = {}
--		local u = (base[#base] - base[1]):normalized()
--		for i=2,#base do
--			apin[#apin+1] = (base[i]-base[1]):dot(u)
--		end
--[[
			aloop[#aloop+1] = awloop[3]
			aloop[#aloop+1] = awloop[4]
			aloop[#aloop+1] = awloop[5]
			aloop[#aloop+1] = awloop[6]
			aloop[#aloop+1] = awloop[1]
			aloop[#aloop+1] = awloop[2]
			aloop[#aloop+1] = awloop[3]
			aloop[#aloop+1] = awloop[4]
			aloop[#aloop+1] = awloop[5]
--			aloop[#aloop+1] = awloop[6]
...
--[[
		if false then
			for i,v in pairs(av) do
				while v.d > csstep and N < 1000 do
						N = N + 1
					csind = csind+1
					psstep = csstep
					csstep = csstep + astep[csind]
						lo('?? sstep:'..csstep..':'..csind)
				end
				--- place vertex
				local s = (v.d-psstep)/astep[csind]
				local p = ap[csind]*(1-s) + ap[csind+1]*s --U.spline2(ap[csind],ap[csind+1],s)
	--				U.dump(v,'?? for_VERT:'..tostring(v.d)..':'..s..':'..tostring(p))
				mbody.verts[v.ind].z = mbody.verts[v.ind].y
				mbody.verts[v.ind].y = p.y
			end
		end
			local c,ir,hit = ap[1],1,true
			local cseg = 1
			local d = 0
			local ahit = {}
			while hit and cseg<#ap-1 and n<100 do
					n = n + 1
					hit = false
				for i = cseg,#ap-1 do
					local list = U.circ2seg(c,ar[ir],ap[i],ap[i+1],U.small_val)
					if #list == 2 then
						-- check projections on segment
					elseif #list == 1 then
							lo('?? HIT:'..i..':'..tostring(list[1]))
						cseg = i
						c = list[1]
						ir = ir + 1
						ahit[i] = c:distance(ap[i])
--						awloop[#awloop+1] = {
--							vec3(d+c:distance(ap[i]))
--						}
					end
					d = d + ap[i]:distance(ap[i+1])
--						lo('?? if_HIT:'..i..':'..#list..':'..tostring(c)..':'..r..':'..ap[i]:distance(ap[i+1])) --..':'..tostring(ap[i])..':'..tostring(ap[i+1]))
--						U.dump(list,'?? if_HIT:'..i..':'..tostring(p1)..':'..tostring(p2))
				end
			end
				U.dump(ahit, '?? aHIT:')
]]
--[[
				if i == doorind then
					step = step + (doorspace - door.w)/2
					ar[#ar+1] = step
					ar[#ar+1] = door.w
					step = (doorspace - door.w)/2
				end
				step = step + (winspace - win.w)/2
				ar[#ar+1] = step
				ar[#ar+1] = win.w
				step = (winspace - win.w)/2
]]
--[[
			local cur = winleft
			for i = 1,nwin do
				...
				if i == doorind then
					-- make door hole
					s = (doorspace - door.w)/2
					w = door.w
					h = door.h
					awloop[#awloop+1] = {vec3(s+cur,-0.1),vec3(s+w+cur,-0.1),vec3(s+w+cur,h),vec3(s+cur,h)}
					-- forward cursor
					cur = cur + doorspace
				end
				-- win hole
				...
				w = win.w
				h = win.h
				awloop[#awloop+1] = {vec3(s+cur,winbot),vec3(s+cur+w,winbot),vec3(s+cur+w,h+winbot),vec3(s+cur,h+winbot)}
--					lo('?? cur_PRE:'..i..':'..cur..':'..winspace)
				cur = cur + winspace
--					lo('?? cur_POST:'..i..':'..cur)
			end
]]
...
--			editor.removeForestItem(fdata, f)
	...
	if false then
		local dirname = editor.getLevelPath()..'bat/'
--		local list = {'b_34619.dae'} -- {'b_23267.dae','b_23268.dae'} --{'b_22397.dae'} -- {'b_21751.dae'} --{'b_21106.dae'} -- {'b_19339.dae'}
		local list = {'b_34619','b_34620'} -- {'b_23267.dae','b_23268.dae'} --{'b_22397.dae'} -- {'b_21751.dae'} --{'b_21106.dae'} -- {'b_19339.dae'}
...
--			lo('?? opened:'..tostring(file))
		file:close()
		local outputFile = io.open(dirname..'emat_2.dae', "w")
		if outputFile then
			outputFile:write(data)
			outputFile:close()
		end
...
--			local pth = dirname..'emat_template.dae' -- 'emat_'..d.id..'.dae'
			d.name = pth
			W.persist({d})
			break
		end
		...
		local dirname = editor.getLevelPath()..'bat/'
		local pth = dirname..'emat_2.dae' -- 'emat_'..d.id..'.dae'
		local xmain = M.xmlOn(pth) --pth)
...
--[[
			local pth = dirname..'emat_29887.dae' -- 'emat_'..d.id..'.dae'
			d.name = pth
			W.persist({d})
]]
			local pth = dirname..'emat_2.dae' -- 'emat_'..d.id..'.dae'
...
-- doesn't work without material set
--			material = 'WarningMaterial',
			colors = {ColorI(255, 0, 255, 255)},
			faces = {
				{v = 0, n = 0, u = 0, c = 0},
				{v = 2, n = 0, u = 2, c = 0},
				{v = 1, n = 0, u = 1, c = 0},
...
--		end
--		local nodes = editor.getNodes(m.obj)
--		meshUp({{
--			vertColors =
--		}})
		return
...
--					U.dump(auv, '?? for_SCAN:')
				...
				forBuilding(desc, function(w, ij)
						--lo('?? for_IJ:'..ij[1]..':'..ij[2])
					if not arc[ij[1]] then arc[ij[1]] = {} end
					arc[ij[1]][ij[2]] = auv[n]
--					arc[ij[1]][ij[2]] = auv['/levels/test_art_v2/bat/shot_test3.png']
					n = n + 1
				end)
					U.dump(arc, '?? for_SCAN:')
...
--				os.execute('magick convert +append '..dirname..'shot_*.png tmerge.png')
				if true then
					W.buildingLOD(desc, arc)
					clear()
				end
			end)

				if true then return end
--			desc.pos = desc.pos + vec3(0,20,0)
--[[
			-- export mesh
			desc.name = 'exp_main'..'_0'
			if false then
				-- shift building
				houseUp(nil, desc.id)
				desc.pos.y = desc.pos.y+20
				houseUp(desc, cedit.mesh, true)
			end
			local floor = desc.afloor[#desc.afloor]
			local arc = coverUp(floor.base)
			local av,auv,af = T.pave(floor.base)
--					U.dump(arc, '??******* for_TOP:'..tostring(desc.afloor))
					U.dump(av, '?? for_AV:'..tostring(floor.top.mat))
					U.dump(auv, '?? for_AUV:')
					U.dump(af, '?? for_AF:'..#af)
			local topmat = scenetree.findObject(floor.top.mat)
					U.dump(topmat, '?? for_MAT:'..tostring(topmat:getPath()))
					lo('?? if_MAT:'..tostring(topmat)..':'..tostring(topmat.baseColorMap))
			local img = imguiUtils.texObj('/levels/test_art_v2/bat/shot_test3.png')
			img = imguiUtils.texObj(topmat.baseColorMap)
					U.dump(img, '?? if_PNG:'..tostring(img.size.x)..'x'..tostring(img.size.y))

			local floor = desc.afloor[#desc.afloor]
			local topmat = scenetree.findObject(floor.top.mat)
			local auv = tex2uv({topmat.baseColorMap,'/levels/test_art_v2/bat/shot_test3.png'})
			local arc = {}
			forBuilding(desc, function(w, ij)
				lo('?? for_IJ:'..ij[1]..':'..ij[2])
				if not arc[ij[1] ] then arc[ij[1] ] = {} end
			...
			arc[#arc+1] = auv[topmat.baseColorMap]
]]
--					U.dump(arc, '?? for_arc:')
...
--				ndtech = M.forNode(xmain, {'COLLADA','library_visual_scenes','visual_scene','node','instance_geometry','bind_material','technique_common'})
--				ndtech.kids[#ndtech.kids+1] = ndinst

--				local ndtech = M.forNode(ndinst, {'bind_material','technique_common'})
...
--			W.persist({desc})
		...
		local dirname = editor.getLevelPath()..'bat/'
		-- parse main mesh
		local xmain = M.xmlOn(dirname..'exp_main'..'_0.dae')
		-- parse lod
		local xlod = M.xmlOn(dirname..dlod_name..'_0.dae')
...
--[[
		...
		cnd = M.toNode(cnd, 'init_from')
		M.text2node(cnd, '/F:/beamng.drive_source/source_environment/common/material/atlas/t_windows_glass_01/t_windows_glass_01_unlit_b.color.png')
		--- geometries
		local nd = M.forNode(xlod, {'COLLADA','library_geometries','geometry'})
			lo('?? if_GEO:'..tostring(nd)..':'..tostring(xlod.kids))
		local ndmain = M.forNode(xmain, {'COLLADA','library_geometries'})
...
--				lo('?? if_SCENE:'..kid.name)
			...
			kid.kids = {} --{nd_maingeo}
			-- append LOD
			kid.kids[#kid.kids+1] = M.forLOD(nd_maingeo, 800, nd)
			kid.kids[#kid.kids+1] = M.forLOD(nd_maingeo, 200, geomain)
--[[
			local dist = '200'
			...
			local nd_ig = M.ofChild(nd_lod, {'instance_geometry'},
				{url='#'..nd.attr['id'], name='e_lod.1_L'..dist}
			)
...
--		local nodevs = M.forNode(xmain, {'COLLADA','library_visual_scenes','visual_scene'})
--		M.toNode(nodevs, 'node', {id='ex_main_1_L200',name='ex_main.1_L200'})

		-- save DAE
		M.xml2file(xmain, dirname..'exp_main'..'.dae')
...
local function meshApply(tp, ind) -- ind is 0-based
		lo('>> meshApply:'..tp..':'..#daePath[tp]..' ind:'..tostring(ind)..':'..tostring(cedit.forest)..':'..tostring(cedit.fscope)..':'..tostring(out.inhole), true)
...
--					U.dump(forspline.aforest, '?? for_SPLINE:'..cedit.forest..':'..cedit.fscope..':'..tp)
				for i,d in pairs(forspline.aforest) do
					if d.type == tp then
						if cedit.fscope ~= 1 or d.key == cedit.forest then
							d.dae = dae
						end
					end
				end
...
--												lo('?? for_I:'..iw)
												cedit.cval['DragPos'].dend[#cedit.cval['DragPos'].dend+1] = {ind=iw,pos=desc.afloor[i].base[iw]}
											end
--											indrag = true
--												_dbdrag = true
											return
...
--										s.base[k] = s.base[k]+dp --vec3(1,0,0)
										...
										if k==1 then
											isave = 1
											iw = cedit.cval['DragPos'].dend[1].ind
										end
										if k==#s.base then
											isave = 2
											iw = U.mod(cedit.cval['DragPos'].dend[2].ind,#desc.afloor[i].base)
										end
--												U.dump(cedit.cval['DragPos'].dend,'?? if_base:'..iw..':'..tostring(isave))
										if iw then
											desc.afloor[i].base[iw] = cedit.cval['DragPos'].dend[isave].pos + dp
											baseOn(desc.afloor[i], desc.afloor[i].base, i, desc)
										end
...
--	if out.dyell and #out.dyell == 1 and out.dyell[1].noz then
--		lo('?? drag_SPLINE:')
--	else
...
--			lo('?? for_IJ:'..ij[2])
			if not tonumber(ij[2]) then
				...
				local p = ray.pos + d*ray.dir
				local pdir = U.proj2D(ray.dir)
--					out.avedit[#out.avedit+1] = p
				if cedit.cval['DragPos'].cpos then
					local vp = U.proj(p-cedit.cval['DragPos'].cpos, pdir)
	...
	--						d.p = d.p + vec3(0,-0.2)
							d.update = true
							desc.aspline[ij[1]][ij[2]].fupdate = i
							houseUp(desc, cedit.mesh, true)
							break
						end
					end
...
--						end
						reg[#reg+1] = reg[1]
						s.areg = {reg}
--						s.areg = {{a,a+u,b,b-u,a}}
						out.ahole = s.areg[1] -- {a,a+u,b,b-u,a}
						break
					end
				end
			end
...
--                    _dbdrag = true
		...
		if false and U._PRD == 0 and scope == 'wall' then
			if not cedit.cval['DragPos'] then
...
--            U.dump(incorner, '?? drag_for_V:'..tostring(incorner))
		elseif false and U._PRD == 0 then
			W.selectionTurn(dx*0.02, cedit.cval['DragRot'].center)
		end
		if toupdate then
			houseUp(adesc[cedit.mesh], cedit.mesh, true)
			markUp()
		...
		elseif cedit.cval['DragRot'] and scope == 'floor' then
			...
			for i = 0,8 do
				...
				if math.abs(da) < 0.07 then
					...
					W.selectionTurn(da, cedit.cval['DragRot'].center)
					houseUp(adesc[cedit.mesh], cedit.mesh, true)
					markUp()
					break
				end
...
--    rayCast = cameraMouseRayCast(false)
	...
	local nm = rayCast.object.name
			lo('??******************* mup:'..tostring(rayCast.object.shapeName)..':'..tostring(nm)..':'..tostring(scope)..':'..tostring(cedit.mesh), true)
	if rayCast.object.shapeName then
		W.forJSON(rayCast.object) --rayCast.object.name) --rayCast.object.shapeName)
		return
	end
	if not nm then
...
--					lo('?? forestAPPEND:'..tostring(out.preforest)..':'..tostring(cedit.fscope)..':'..key..':'..tostring(dforest[key]))
				if out.preforest then
					cedit.aforest = {out.preforest}
					out.preforest = nil
				end
				if #U.index(cedit.aforest,key) == 0 then
					cedit.aforest[#cedit.aforest+1] = key
				end
...
--                desc.selection[tij[1] ][tij[2] ] = true
			...
			markUp()
			return ij
		elseif desc and nm ~= forestName then
--				lo('??******* sel_OFF:'..nm)
			desc.selection = nil
		end
	end
...
--        local rCast = cameraMouseRayCast(false)
			lo('?? for_forest:'..tostring(scope)..':'..tostring(dforest[key])..':'..tostring(key)..'/'..tostring(cedit.forest)..':'..tostring(cedit.fscope)) --..':'..tostring(rCast.object.name)) --..':'..tostring(dforest[key].mesh)..':'..tostring(cedit.fscope))
...
--					out._dbgcheck = 1
		...
		elseif not (dforest[key].type == 'door' and scope == 'building') then
				lo('?? tof2_:'..key..' fscope:'..tostring(cedit.fscope)..':'..dforest[key].type..':'..tostring(dforest[key].mesh)..':'..tostring(cedit.mesh)..':'..tostring(#cedit.aforest), true)
...
--						lo('?? to_SSEL:')
					if not desc.selection then desc.selection = {} end
					if not desc.selection[i] then desc.selection[i] = {} end
					desc.selection[i][#desc.selection[i]+1] = key
					markUp()
					break
				end
			end
...
--		if out.dyell and #out.dyell then lo('?? oU:') end
...
--            end
	...
	if out.dyell then -- and (W.inView() or U._PRD==0) then
--			lo('?? for_DYELL:')
		for i,row in pairs(out.dyell) do
--			if row.noz then
...
--                        U.dump(out.acorner, '?? acorn:')
				...
				elseif row.noz then
					-- handle spline nodes
					for i=1,#pth do
						if U.angDistVert(pth[1],pth[2]) then
							incorner = i
							break
						end
					end
				...
				for i=1,#pth-1 do
					local clr = ColorF(1,1,0.5,row.noz and 0.6 or 1)
					local w = 4
...
--                        debugDrawer:drawLineInstance(pth[i], pth[i+1], 4, ColorF(0.5,1,0.5,0.1))
--                    end
					debugDrawer:drawLineInstance(pth[i], pth[i+1], w, clr, not row.noz)
	--                debugDrawer:drawLine(pth[i], pth[i+1], ColorF(1,1,0.5,0.5), 6)
				end
				::continue::
			end
...
--                        incorner = false
	...
	-- SPLITTED WALL MIDDLE
					if scope == 'wall' and U._PRD == 0 and cij then
...
local function toggle(tp)
	if not daePath[tp] or tableSize(daePath[tp]) == 0 then return end
...
--[[
		if out._dbgcheck then
			out._dbgcheck = out._dbgcheck+1
			if out._dbgcheck>1000 then
				out._dbgcheck = false
			else
...
--			lo('??^^^^^^^^^^^^_______________^^^^^^^^^^^ ifForest:'..#cedit.aforest..':'..tostring(W.forSpline()))
			if #cedit.aforest>0 then
				lo('?? dftp:'..dforest[cedit.aforest[1]].type)
			end
			out._dbgcheck = false
		end
	if out.inhole then
...
--[[
	if #cedit.aforest>0 then
		if #U.index(atp, dforest[cedit.aforest[1] ].type)>0 then
--			lo('?? for_TRUE:')
			return true
		end
	else
]]
	if #cedit.aforest>0 and not atp then
		return true
	end
	if #cedit.aforest>0 and atp and atp[1]=='win' then
--		return true
...
--[[
	...
	if #cedit.aforest>0 then
		...
		if dforest[cedit.aforest[1]] and #U.index(atp, dforest[cedit.aforest[1]].type)>0 then
--			lo('?? for_TRUE:')
			return true
		end
	elseif cedit.forest then
...
W.persist = function(ahouse)
	if not ahouse then ahouse = adesc end
    local dirname = editor.getLevelPath()..'bat/'
...
--		lo('?? daeExport:'..#aid..':'..#afi..':'..tostring(tableSize(editor.selection))) --..':'..tostring(editor.selection.forestItem))--..':'..tostring(aid[1])..':'..tostring(editor.selection.object.id)..':'..tostring(tableSize(editor.selection)))
--		scope = nil
--		local fpath = dirname..(desc.name or 'exp_'..desc.id)..'.dae'
		local fpath = desc.name or (dirname..'exp_'..desc.id..'.dae')
...
--		lo('?? onVal:'..key..':'..tostring(val))
	...
	if false then
	elseif key == 'win_spacefit' then
		local s,ij = W.forSpline()
		if s then
			s.spacefit = true
		end
--		if s then W.splineUp(adesc[cedit.mesh],ij[1],ij[2],s) end
	elseif key == 'wall_spline' then
		local desc = adesc[cedit.mesh]
...
--			if true then return end
		W.building4test(W.ui.build_gen, W.ui.build_radius, W.ui.gen_center)
--[[
		local pth = U.path2disk({editor.getLevelPath(), 'bat', 'test'})
		...
		local ashape = {'l_shape',
			...
			L = 128
--		tb = extensions.editor_terrainEditor.getTerrainBlock()
		for i=1,W.ui.build_gen do
			local base = shape2base(ashape[math.random(#ashape)])
			local x = math.random(L) - L/2
		...
		onVal('dae_exp')
		return
	elseif key == 'gen_inst' then
--{"name":"LevelObjects","class":"SimGroup","persistentId":"ad3f71ba-89b5-4dc3-94e2-3403b4906dc8","__parent":"MissionGroup"}
--{"name":"PlayerDropPoints","class":"SimGroup","persistentId":"47166035-d3cf-466f-9153-e42befc5e531","__parent":"MissionGroup"}
			...
			forBuilding(d, function(w, ij)
				for pth,list in pairs(w.df) do
					for _,id in pairs(list) do
						...
						local scale = item:getScale()
						local trans = item:getTransform()
						local rot = trans:toEuler()

						local fobj = ddae[pth].fo
						local vang = rot
						...
						df[d.id].list[#df[d.id].list+1] = {obj = fobj, mtx = mtx, pos=pos, scale=sc}
--				local item = fdata:createNewItem(fobj, mtx, sc)
					end
				end
			end)
		...
		for id,list in pairs(dpos) do
			for _,p in pairs(list) do
				for _,o in pairs(df[id].list) do
					local mtx = o.mtx
					mtx:setPosition(o.pos+p)
					item = fdata:createNewItem(o.obj, o.mtx, o.scale)
					key = item:getKey()
					dforest[key] = {item = item} --, mesh = nil, type = daedesc.type}
				end
			end
		end
--[[
...
--			W.persist()
			local dirplus = W.out.inperftest and 'test/' or ''
			local batdir = editor.getLevelPath()..'bat/'..dirplus
			local list  = {}
			local jlist = {}
			-- to DAE
			scope = nil
...
--					M.toDAE(d.data, batdir..nm..'.dae')
					...
					jlist[#jlist+1] = {
						class='TSStatic',
						persistentId='0ee68748-c2ce-461e-805f-743500c4f'.._,
						__parent='bat',
						isRenderEnabled=true,
						shapeName=batdir..nm..'.dae',
					}
				end
--[[
				local om = createObject('TSStatic')
...
--						lo('?? for_DAE:'..tostring(om.obj)) --..':'..om.__parent)
			...
			--TODO: to items.level
			U.path2disk({editor.getLevelPath()..'/main/MissionGroup/', 'bat'})
			local dname = editor.getLevelPath()..'/main/MissionGroup/bat/'
			local fname = 'items.level.json'
			local outputFile
			if W.out.inperftest then
				-- overwrite file
				outputFile = io.open(dname..fname, "w")
				outputFile:close()
			end
			outputFile = io.open(dname..fname, "r")
...
--[[
				require 'lfs'
				lfs.mkdir(dname)
				local ok, err, code = os.rename(dname, dname)
				if not ok then
					lo('!! NO_BAT_DIR:')
				end
]]
			...
			}, {editor.getLevelPath(), '/main/MissionGroup/'}, 'items.level.json', 'a')
		end
		return
			...
			if not tonumber(ij[2]) then
				for _,d in pairs(desc.aspline[ij[1] ][ij[2] ].aforest) do
					if d.key == cedit.forest then
						d.update = true
						if not cedit.cval['DragPos'] then
							cedit.cval['DragPos'] = {p = d.p, val = val}
							return
						end
...
--						U.dump(s.aforest, '?? if+INFOOR:'..cedit.forest)
--						U.dump(cedit.aforest, '?? cAFOOR')
--					U.dump(desc.seletion[ij]'?? for_SPL:'..ij[1]..':'..ij[2]..':'..cedit.forest)
--[[
		if false then
			local s = W.forSpline()
			if s then
					lo('??+++++++++++++++ ifCF:'..tostring(cedit.forest))
				for i,d in pairs(s.aforest) do
					if d.key == cedit.forest then
						lo('??^^^^^^^^ HITF:')
						if not cedit.cval['DragPos'] then
...
--								return
	...
	elseif key == 'win_bottom' then -- or key == 'win_u' then
		forBuilding(desc, function(w, ij)
			if not tonumber(ij[2]) then
...
--        lo('??^^^^^^^^^^^^^^^ onV_HU:')
	local ftype
	if cedit.forest and dforest[cedit.forest] then
		ftype = dforest[cedit.forest].type
	end
	houseUp(desc, cedit.mesh, true)
--        lo('?? onVal_out2:'..tostring(cedit.forest)..':'..tostring(W.ifForest({'win'}))..':'..tostring(W.ifForest({'pillar'}, true)), true)
	...
	if ftype then
		local forestset
		forBuilding(desc, function(w)
			if not forestset and ({win=0,door=0})[ftype] then
				cedit.forest = w.df[w[ftype]] and w.df[w[ftype]][1] or nil
				if cedit.forest then forestset = true end
			end
		end)
	end
