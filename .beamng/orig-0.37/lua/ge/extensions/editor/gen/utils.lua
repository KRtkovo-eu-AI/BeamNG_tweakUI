...
U.angDistVert = function(a,b,sense)
	if not sense then sense = 0.015 end

	local rayCast = cameraMouseRayCast(false)
	if not rayCast then return end

	local campos = core_camera.getPosition()
	local vp = U.proj2D(a-campos)
	local torayhit = rayCast.pos - campos
	local p = campos + torayhit:normalized()*vp:length()*torayhit:length()/U.proj2D(torayhit):length() --/torayhit.z
	local d,pvert = U.toLine(p, {a, b})
	if U.between(a, b, {pvert}) then
		local dang = U.angDist(pvert)
		if dang and dang < 0.015 then
			return true
		end
	end
end
...
U.angDistSeg = function(line, b)
	if not b then
		local rayCast = cameraMouseRayCast(false)
		if rayCast then b = rayCast.pos end
	end
	local d,p = U.toLine(b, line)
		lo('?? angDistSeg:'..tostring(d)..':'..tostring(p)..':'..tostring(b)..':'..U.vang(p,b))
...
U.shape2name = function(shapeName)
	local astep = U.split(shapeName, '/', true)
	local nm = U.split(astep[#astep], '.', true)
	nm = nm[1]
...
U.toFile = function(obj, pth, fname, md)
	local s = U.path2disk(pth)
	local file
...
--			lo('?? write:'..#obj..':'..cnt)
	...
	file = io.open(s..fname, 'w')
	file:write(cnt..obj)
	if file then file:close() end
end


U.fromFile = function(fname)
...
U.circ2seg = function(c,r,a,b,sence)
	if not sence then sence = 0 end
	local A,B,C = a.y-b.y,b.x-a.x,a.x*b.y-a.y*b.x
	local p1,p2
	local c2,c1,c0
	if math.abs(B)<small_val then
		lo('?? circ2line_SMALL_B2:'..B..':'..tostring(a)..':'..tostring(b))
		c2,c1,c0 =
			A*A+B*B,
			2*B*C+2*A*B*c.x-2*A*A*c.y,
			C*C+2*A*C*c.x-A*A*(r*r-c.x*c.x-c.y*c.y)
	else
		c2,c1,c0 =
			A*A + B*B,
			2*A*C + 2*A*B*c.y - 2*B*B*c.x,
			C*C + 2*B*C*c.y - B*B*(r*r - c.x*c.x - c.y*c.y)
	end
	local d = c1*c1-4*c2*c0
--			lo('?? circ2seg_1:'..c2..':'..c1..':'..c0..' d:'..d)
	if d >= 0 then
		local x1,x2 = (-c1+math.sqrt(c1*c1-4*c2*c0))/(2*c2),(-c1-math.sqrt(c1*c1-4*c2*c0))/(2*c2)
...
--		lo('?? if_BETW:'..tostring(isb)..''..)
	...
	if p2 and U.between(a,b,{p2},sence) then
		across[#across+1] = p2
	end
...
U.ray2seg = function(a, b, c, d, sense)
	if not sense then sense = 0 end

	local p,s = line2seg(a,b,c,d,sense)
	if s then
...
-- spline between circular approximation of triples of points
U.circSpline = function(base)
	local acirc = {}
	for i=2,#base-1 do
		local c1,c2 = (base[i]+base[i-1])/2,(base[i+1]+base[i])/2
		local v1,v2 = U.vturn(base[i]-base[i-1],math.pi/2),U.vturn(base[i+1]-base[i],math.pi/2)
...
-- use pair of circles with share s to place point p
U.splineC = function(p, c1, c2, s)
	local a = c1.c + c1.r*(p-c1.c):normalized()
	local b = c2.c + c2.r*(p-c2.c):normalized()

	return U.spline2(a,b,s)
end

-- connect polygon nodes using circular spline
U.path2spline = function(base)
...
--        U.dump(ap, '>> paraSpline:'..#ap)
	local apar = {}
	for i = 2,#ap-1 do
		local p1 = {x = ap[i-1].x or ap[i-1][1], y = ap[i-1].y or ap[i-1][2]}
		local p2 = {x = ap[i].x or ap[i][1], y = ap[i].y or ap[i][2]}
		local p3 = {x = ap[i+1].x or ap[i+1][1], y = ap[i+1].y or ap[i+1][2]}
...
U.polySpline = function(base)
	local cma,curv = 0
	local adir = {}
	for i=2,#base-1 do
		local va,vb = (base[i]-base[i-1]):normalized(), (base[i+1]-base[i]):normalized()
		adir[#adir+1] = U.vturn(vb-va, -math.pi/2)
		curv = U.curv(base[i-1], base[i], base[i+1])
		if curv > cma then
			cma = curv
		end
	end
	...
	adir[#adir+1] = U.vturn(base[#base]-base[#base-1], ang):normalized()

	return cma,adir
end

-- spline between values a,b for a point at i/n in between
