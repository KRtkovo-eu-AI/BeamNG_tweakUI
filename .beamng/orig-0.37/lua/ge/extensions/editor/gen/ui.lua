...
local count_inp
local header = 'Building Architect (WIP)' --'City Editor'
...
local function forHelp(W, R)
	...
	row[#row + 1] = {'Shift-Click', insector and 'pick sector' or 'add/remove center'}
--	row[#row + 1] = {'Alt-Click', 'road/exit '..('start/end')}
	...
	HTree = {{}, {}, {}, {}}
	local row = HTree[1]
	row[#row + 1] = {'Ctrl-Click', 'new building'} -- W.overWall()==1 and 'append to selection' or 'new building_1'}
	if W.out.axis then
...
--		print('?? if_LAT:'..tostring(editor.beginWindow('LAT', 'WIP_Landscape_Gen')))
	...
	if editor.beginWindow('TEST', 'WIP_Performance_Test') then
		columnRight(164)
...
--        im.Separator()
		...
		----------- SCOPE SELCTOR --------------------
		----------------------------------------------
		local rightPanelWidth = U._HERE and 90 or 2
...
--                        buttonC('width_fit', 'hole_fitmesh', nil, im.ImVec2(30, 30))
					elseif cforest then
--							print('?? UI_cfor:'..tostring(cforest))
...
--							columnRight(60)
							...
							if spline then -- desc.selection and tableSize(desc.selection) == 1 then
								...
								slider('u-Position', 'win_u', {-5, 5})
								slider('v-Position', 'win_v', {-3, 3})
							else
								-- z-pos
							...
							if W.out.fscope ~= 1 then
								...
								im.NextColumn()
								if spline then
									slider('Padding', 'win_padding', {0.02, 5})
								else
									-- space
