...
local function getMtlIdByName(matName) --terrain layers
  -- log('I', 'terrainEditor', '[DEBUG] getMtlIdByName called with: "' .. (matName or "nil") .. '"')
  -- log('I', 'terrainEditor', '[DEBUG] Available materials: ' .. #(var.paintMaterialNamesArray or {}) .. ' total')
  ...
  if var.paintMaterialNamesArray then
    for k, mat in ipairs(var.paintMaterialNamesArray) do
      ...
      if mat == matName then
        -- log('I', 'terrainEditor', '[DEBUG] Found match at index ' .. k .. ' for material "' .. matName .. '"')
        return k, matName
      end
    end
  end

  local fallbackName = (paintMaterialProxies and paintMaterialProxies[1] and paintMaterialProxies[1].internalName) or "unknown"
...
local function terrainImporter_Accept()
  ...
  for i,map in ipairs(terrainImpExp.textureMaps) do
    ...
    local expectedMaterial = var.paintMaterialNamesArray and var.paintMaterialNamesArray[map.materialId[0] + 1] or "unknown"
