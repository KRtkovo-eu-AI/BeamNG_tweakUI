  ...
  if editor_crawlEditor and editor_crawlEditor.markAsDirty then
    ...
    for i, sp in ipairs(allStartingPositions) do
      if sp == startingPosition then
        editor_crawlEditor.markAsDirty("startingPosition", i)
        break
      end
    end
  end
...
function C:input(mouseInfo)
  ...
  -- Handle direct placement with Ctrl+click and Shift+click
  if not editor.isAxisGizmoHovered() and mouseInfo.down and mouseInfo._downPos then
    local mousePos = mouseInfo._downPos

    -- Ctrl+click to place area position
    if editor.keyModifiers.ctrl then
      ...
      editor.history:commitAction("Place Starting Position Area",
        {startingPosition = self.startingPosition, old = {position = self.startingPosition.transform.position, rotation = self.startingPosition.transform.rotation, radius = self.startingPosition.transform.radius}, new = {position = self.startingPosition.transform.position, rotation = self.startingPosition.transform.rotation, radius = self.startingPosition.transform.radius}},
        setTransformUndo, setTransformRedo)
      markStartingPositionAsDirty(self.startingPosition)
      return
    end

    -- Shift+click to place icon position
    if editor.keyModifiers.shift then
      self.startingPosition.iconPosition = mousePos
      editor.history:commitAction("Place Starting Position Icon",
        {startingPosition = self.startingPosition, old = self.startingPosition.iconPosition, new = self.startingPosition.iconPosition, field = 'iconPosition'},
        setFieldUndo, setFieldRedo)
      markStartingPositionAsDirty(self.startingPosition)
      return
    end
  ...
  -- Handle transform selection (only if not clicking on gizmo)
...
function C:dragging()
  ...
  elseif editor.getAxisGizmoMode() == editor.AxisGizmoMode_Rotate and self.currentTransform == "area" then
    local newRot = quat(editor.getAxisGizmoTransform():getRotation())
    self.startingPosition.transform.rotation = newRot
  end
