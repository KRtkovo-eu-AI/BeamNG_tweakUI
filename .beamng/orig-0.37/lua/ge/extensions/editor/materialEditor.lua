...
local function getMaterials()
  ...
  if editor.selection and options then
...
local function onEditorGui()
  ...
  if editor.beginWindow(toolWindowName, "Material Editor") then
    ...
    if im.BeginChild1("MATERIALEDITORMAIN") then
      ...
      if im.Button("From Scene Selection") then
        getMaterials()
      ...
      im.tooltip("Shows Only Materials From the Selected Object(s) (it will deselect objects)")

      im.SameLine()

      if im.Button("All Materials") then
        editor.clearObjectSelection()
        if editor_forestEditor then
          editor_forestEditor.clearForestItemsSelection()
        end
      ...
      if true == filteredBySceneSelection then
        im.SameLine()
        im.TextColored(im.ImVec4(1, 1, 0, 1), "[From Scene Selection]")
      end
