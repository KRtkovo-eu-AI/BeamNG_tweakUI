...
local function pickBeam()
  ...
  for _, i in ipairs(beamsAvaliable) do
    ...
    if beamTypesRendering[beamType][0] then
      ...
      local beamPos1 = vEditor.vehicle:getNodeAbsPosition(beam.id1)
      local beamPos2 = vEditor.vehicle:getNodeAbsPosition(beam.id2)
...
local function renderPickedNodesTree()
  if not pickedNodes then return end
