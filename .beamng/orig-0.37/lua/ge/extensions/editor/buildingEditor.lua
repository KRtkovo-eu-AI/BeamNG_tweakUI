...
--		print('>>___________________ ce.reload:'..#apack)
...
--		print('??************ bE.reload:'..tostring(mode)..':'..tostring(UU._MODE))
	if UU._PRD == 0 and mode ~= 'conf' then
		unrequire(apack[5])
		local Tst = rerequire(apack[5])
		Tst.inject({E = M, W = W, UI = UI,  UU = UU})
	end
...
--	R = rerequire(apack[4])
	...
	if not soft then
--			print('??_______________ for_up:')
		W.up(D)
	end

	editor.showWindow('BAT')
	if UU._PRD == 0 then --and ({conf=0,ter=0})[UU._MODE] then
		editor.showWindow('LAT')
	end
]]
end
