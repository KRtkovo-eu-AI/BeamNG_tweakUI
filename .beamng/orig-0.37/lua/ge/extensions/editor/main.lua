...
local function setEditorActiveInternal(activate, safeMode)
  ...
  core_input_actionFilter.setGroup('editorBlockedActions', blockedInputActions)
  core_input_actionFilter.addAction(0, 'editorBlockedActions', activate)
  ...
  if activate and not activated then
    ...
    if editor.getPreference("ui.general.muteGameAudioOnEditorActivate") then
      editor.muteAudio(true)
    end
  else
    if activated and not activate then
      ...
      editor.setCameraSpeed(editor.savedCameraSpeed)
      ...
      if editor.getPreference("ui.general.muteGameAudioOnEditorActivate") then
        editor.muteAudio(false)
      end
      editor.resetAllDirtyTools()
    end
  end
end
...
local function onClientStartMission()
  ...
  setEditorActiveInternal(M.active)
end
