...
local hiddenClasses = {
  MeshRoad = "Mesh Road Editor",
  DecalRoad = "Decal Road Editor",
  River = "River Editor"
}
...
local function createObjectInstance(classname, createObjectItem)
  ...
  createObjectItem = createObjectItem or getObjectCreateItem(classname)
  if instance and createObjectItem then
    if createObjectItem.buildFunc then createObjectItem.buildFunc(instance) end
  ...
  if createGroups[createObjectGroupIndex].objectClasses.otherClasses == true then
    ...
    for i, item in ipairs(createGroups[createObjectGroupIndex].objectClasses) do
      ...
      if editor.uiIconImageButton(item.icon or editor.icons.stop, nil, nil, nil, bgColor) then
        ...
        currentClassInstance = createObjectInstance(item.classname, item)
...
-- @returns the entire create group table
local function getAllObjectCreateGroups()
  return createGroups
end

--- Initializes all groups; this exists so it can be called even if the editor is not active
local function initGroups()
  ...
  -- attaches these functions to the editor
  editor.addObjectCreateGroup = addObjectCreateGroup
  editor.getObjectCreateGroup = getObjectCreateGroup
  editor.getAllObjectCreateGroups = getAllObjectCreateGroups
  editor.makeCreateObjectItem = makeCreateObjectItem
  editor.getObjectCreateItem = getObjectCreateItem
  editor.createObjectInstance = createObjectInstance
