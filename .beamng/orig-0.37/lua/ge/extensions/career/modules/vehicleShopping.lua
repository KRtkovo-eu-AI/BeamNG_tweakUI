...
local function updateVehicleList(fromScratch)
  ...
  for i = #vehiclesInShop, 1, -1 do
    ...
    if offerTime > vehicleInfo.offerTTL then
      ...
      if vehicleInfo.soldViewCounter > 2 then
  ...
  for _, seller in ipairs(sellers) do
    ...
    if onlyStarterVehicles then
      ...
      local numberOfVehiclesToGenerate = math.min(math.floor((currentTime - sellersInfos[seller.id].lastGenerationTime) / dealershipTimeBetweenOffers), 10)
