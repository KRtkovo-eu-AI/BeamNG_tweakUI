...
local function onPlayerCameraReady()
  ...
  local trafficAmount, parkedVehiclesAmount = nil, nil
  ...
  if forceTrafficLoading ~= nil then
    ...
    elseif type(forceTrafficLoading) == 'table' then
      if type(forceTrafficLoading.traffic) == 'boolean' then
        loadTraffic = forceTrafficLoading.traffic
      elseif type(forceTrafficLoading.traffic) == 'number' then
        trafficAmount = forceTrafficLoading.traffic
        loadTraffic = forceTrafficLoading.traffic > 0
      end
      if type(forceTrafficLoading.parkedVehicles) == 'boolean' then
        loadParkedVehicles = forceTrafficLoading.parkedVehicles
      elseif type(forceTrafficLoading.parkedVehicles) == 'number' then
        parkedVehiclesAmount = forceTrafficLoading.parkedVehicles
        loadParkedVehicles = forceTrafficLoading.parkedVehicles > 0
      end
    end
  ...
  if M.state.freeroamActive and loadTraffic then
    log('I', logTag, 'Now spawning traffic for freeroam mode ('.. (loadParkedVehicles and 'with' or 'without') .. ' parked vehicles)')
    if loadParkedVehicles then
      gameplay_parking.setupVehicles(parkedVehiclesAmount)
    end
    gameplay_traffic.setupTraffic(trafficAmount, nil, {activeAmount = trafficAmount})
  end
end
