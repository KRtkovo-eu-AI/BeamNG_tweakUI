...
function C:rayIntersectsZone(rayPos, rayDir)
  local fenceVertices = {}
  for _, v in ipairs(self.vertices) do
    ...
    if self.usePlanes then
      if self.top.active then
        b = self:vertexPlaneIntersection(v.pos,self.top)
        c = self:vertexPlaneIntersection(self.vertices[v.next].pos,self.top)
      end
      if self.bot.active then
        a = self:vertexPlaneIntersection(v.pos,self.bot)
        d = self:vertexPlaneIntersection(self.vertices[v.next].pos,self.bot)
      end
    end
    local vertices = {
      a = a,
...
function C:drawDebug(drawMode, clr, customHeight, customDown, nearCam, drawDistance)
  ...
  local isVisible = self:aabbCheck(camPos)
  ...
  if #self.vertices > 1 then
    ...
    self.fenceDown = customDown or -50
    self.fenceHeight = customHeight or 5
    self.usePlanes = usePlanes
...
function C:drawFence(from, to, clrI, down, up, usePlanes, nearCam, drawDistance, shapeAlpha)
  ...
  camPos = camPos or core_camera.getPosition()
  if not nearCam or
...
function C:makeHighResolutionFence(maxStep)
  local newVerts = {}
  local vCount = #self.vertices
  maxStep = math.max(1, maxStep or 3) -- in metres
...
function C:zoneArea()
  local numPoints = #self.vertices

  if numPoints < 3 then
    return 0 -- A polygon with less than 3 points has no area
  end

  local area = 0
  for i = 1, numPoints do
    local currentPoint = self.vertices[i].pos
    local nextPoint = self.vertices[(i % numPoints) + 1].pos -- Wrap around to the first point

    area = area + (currentPoint.x * nextPoint.y) - (currentPoint.y * nextPoint.x)
  end
