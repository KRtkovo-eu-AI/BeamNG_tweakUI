...
local function getTotalImpactDamage(crashData)
  local totalImpactDamage = 0
  if crashData.preImpactData then
    for _, frameDamage in pairs(crashData.preImpactData.frameDamages) do
      totalImpactDamage = totalImpactDamage + frameDamage.newDamage
    end
  end
  if crashData.currentImpactData then
    for _, frameDamage in pairs(crashData.currentImpactData.frameDamages) do
      totalImpactDamage = totalImpactDamage + frameDamage.newDamage
    end
  end
  return totalImpactDamage
end
...
local function managedImpactAndDetectImpacts(vehData)
  ...
  local damageSum = crashData.vehObj:getSectionDamageSum() or 0
  ...
  if newDamageSum > 0 or newDamage > 0 then
    -- Reuse table to avoid garbage creation
...
local function drawDebug(vehData)
  ...
  if crashData.currentCrashImpacts then
    for impactIndex, impactData in pairs(crashData.currentCrashImpacts) do
      ...
      debugDrawer:drawTextAdvanced(impactData.frameDamages[1].vehPos, string.format("#%i Impact speed: %i kph", impactIndex, impactData.impactSpeed), White, true, false, BlackBackground)
      -- draw the end of impact
      debugDrawer:drawSphere(impactData.frameDamages[#impactData.frameDamages].vehPos, 0.4, RedTransparent)
      debugDrawer:drawTextAdvanced(impactData.frameDamages[#impactData.frameDamages].vehPos, string.format("#%i Damage: %i", impactIndex, impactData.totalDamage), White, true, false, BlackBackground)
      if impactData.damageStateDiff then
        debugDrawer:drawTextAdvanced(impactData.frameDamages[#impactData.frameDamages].vehPos, impactData.damageStateDiff.mostDamagedLocation, White, true, false, BlackBackground)
      end

      -- draw average position of the impact
      debugDrawer:drawSphere(impactData.averagePos, 0.25, BlueTransparent)
      debugDrawer:drawTextAdvanced(impactData.averagePos, string.format("#%i", impactIndex), White, true, false, BlackBackground)
      ...
      for frameDamageIndex, frameDamageImpact in pairs(impactData.frameDamages) do
        ...
        debugDrawer:drawText(frameDamageImpact.vehPos, string.format("%i", frameDamageImpact.newDamage), Black)
...
local function addTrackedVehicleById(vehId_, crashSettings, owner)
  ...
  trackedVehIds[vehId_] = {
    ...
    totalCrashTime = 0,
    vehId = vehId_,
    vehObj = scenetree.findObjectById(vehId_),
