...
M.loadCrawlData = function(filename)
  ...
  log('I', logTag, 'Attempting to load crawl data from: ' .. tostring(filename))
  ...
  log('I', logTag, 'Loaded crawl data with ' .. #crawlData.trails .. ' trails')
  return crawlData
end
...
M.getStartingPositionById = function(startingPositionId)
  if not startingPositionId then
    log('E', logTag, 'No starting position ID provided')
    return nil
  end
...
local function ensurePlayerSaveDirectories()
  ...
  if not FS:directoryExists(playerSavePathFreeroam) then
    ...
    log('I', logTag, 'Created crawl trails save directory: ' .. playerSavePathFreeroam)
  end
end
...
local function addNewPlayerScore(trailId, time, penaltyPoints, damage)
  local trail = M.getPlayerCrawlTrailById(trailId)
  if not trail then
    log("E", logTag, "Trail not found for ID: " .. tostring(trailId))
    return
  end
...
M.cleanup = function()
  clearCache()
  log('I', logTag, 'Save system cleanup completed')
end
