...
function C:evaluatePacenoteEvents(pacenote, speedMs)
  ...
  vehDistToFinish = self.route.path[1].distToTarget
  ...
  if speedMph > self.scaleMinSpeedMph then
    scaleFactor = math.min((speedMph - self.scaleMinSpeedMph) / (self.maxMultiplier - self.minMultiplier), 1.0)
    speedMultiplier = self.minMultiplier + ((self.maxMultiplier - self.minMultiplier) * scaleFactor)
  end
  audioLenTotal = pacenote:audioLenTotal()
  audioLenTotalScaled = audioLenTotal * self.audioLenScaler
  ...
  for percent, hit in pairs(event.cornerPercentHit) do
    distPercent = pacenote:getCachedLength() * percent
    absDistPercent = wpCs:getOriginalDistToTarget() - distPercent
