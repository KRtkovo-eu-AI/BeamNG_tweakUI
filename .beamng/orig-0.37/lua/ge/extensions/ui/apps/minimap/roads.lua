...
M.drawRoads = function(p, radius, td, debugSettings, camPos, scale, dpi  )
  ...
  if not kdNodes then
    ...
    --table.clear(cachedQueryResult)

    local idx = 1
    for lIdx in kdNodes:queryNotNested(camPos.x-radius, camPos.y-radius, camPos.x+radius, camPos.y+radius) do
      cachedQueryResult[idx] = lIdx
      idx = idx + 1
      -- stats.navgraphRoadsVisible = stats.navgraphRoadsVisible + 1
      local n1PosX = links[lIdx  ]
      local n1PosY = links[lIdx+1]
      local n2PosX = links[lIdx+2]
      local n2PosY = links[lIdx+3]

      local s1X, s1Y = worldToMapXY(n1PosX, n1PosY)
      local s2X, s2Y = worldToMapXY(n2PosX, n2PosY)

      tdlineRoundEnd(td, s1X, s1Y, s2X, s2Y, constantWidthBg, constantWidthBg, 0, roadBgTransparentBlack, roadBgTransparentBlack, 0, 0)
    end

    if p then p:add("Roads BG") end

    for i = 1, idx - 1 do
      local lIdx = cachedQueryResult[i]
      -- stats.navgraphRoadsVisible = stats.navgraphRoadsVisible + 1
      local n1PosX = links[lIdx  ]
      local n1PosY = links[lIdx+1]
      local n2PosX = links[lIdx+2]
      ...
      tdlineRoundEnd(td, s1X, s1Y, s2X, s2Y, constantWidth, constantWidth, 0, clr, clr, 0, 0)
    end
