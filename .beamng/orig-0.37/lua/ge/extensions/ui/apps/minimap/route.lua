...
local function drawRoute(route, td, dpi)
  ...
  for i = 1, routeCurrentLength-1 do
    td:lineRoundEnd(routePath[i].x, routePath[i].y, routePath[i+1].x, routePath[i+1].y, 4*dpi, 4*dpi, 0, clrNavBg, clrNavBg, 0, 0)
  end
  for i = 1, routeCurrentLength-1 do
    local color = getRouteColor(((startDistance - route[i].distToTarget) + (startDistance - route[i+1].distToTarget))/2)
    td:lineRoundEnd(routePath[i].x, routePath[i].y, routePath[i+1].x, routePath[i+1].y, 2*dpi, 2*dpi, 0, color, color, 0, 0)
  end
