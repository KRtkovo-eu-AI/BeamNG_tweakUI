...
local function drawVehicle(pos, fwd, color, size, asCircle)
  ...
  if asCircle then
    ...
    td:triangle(f.x, f.y, bl.x, bl.y, br.x, br.y, borderWidth, 0, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg)
    td:circle(tmp1.x, tmp1.y, (5*size+2) * dpi, 0, currentStyleColorSet.navBg, currentStyleColorSet.navBg, 0, 0)

    td:triangle(f.x, f.y, bl.x, bl.y, br.x, br.y, 0, 0, color, color, color, color)
    td:circle(tmp1.x, tmp1.y, 5*size * dpi, 0, color, color, 0, 0)
  else
    ...
    td:triangle(f.x, f.y, bl.x, bl.y, bc.x, bc.y, border + 1, 0, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg)
    td:triangle(bc.x, bc.y, br.x, br.y, f.x, f.y, border + 1, 0, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg)

    td:triangle(f.x, f.y, bl.x, bl.y, bc.x, bc.y, 1, 0, color, color, color, color)
    td:triangle(bc.x, bc.y, br.x, br.y, f.x, f.y, 1, 0, color, color, color, color)
  end
end
...
local function drawPlayer(dtReal, dtSim)
  ...
  if debugSettings.drawPlayer then
    ...
    drawVehicle(pos, fwd, currentStyleColorSet.clrFocus, 1, gameplay_walk.isWalking())
  end
  if commands.isFreeCamera() then
    --drawVehicle(camPos, cameraLook, clrOrange, 1, true)
    tmp1:set(cameraLook)
    ...
    td:line(tmp3.x, tmp3.y, tmp2.x, tmp2.y, 0, 4*dpi, strokeWidth, strokeWidth, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg)
    td:line(tmp1.x, tmp1.y, tmp3.x, tmp3.y, 6*dpi, 6*dpi, strokeWidth, strokeWidth, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg, currentStyleColorSet.navBg)

    td:line(tmp3.x, tmp3.y, tmp2.x, tmp2.y, 0, 4*dpi, 0, 0, currentStyleColorSet.clrFocus, currentStyleColorSet.clrFocus, currentStyleColorSet.clrFocus, currentStyleColorSet.clrFocus)
    td:line(tmp1.x, tmp1.y, tmp3.x, tmp3.y, 6*dpi, 6*dpi, 0, 0, currentStyleColorSet.clrFocus, currentStyleColorSet.clrFocus, currentStyleColorSet.clrFocus, currentStyleColorSet.clrFocus)
  end
...
local function drawOtherVehicles(dtReal, dtSim)
  ...
  for otherVId, otherVeh in activeVehiclesIterator(vehIteratorCtx) do
    if otherVId ~= currentPlayerId then
      local draw = false
      ...
      drawVehicle(pos, fwd, vehColor, canUse and 0.8 or (policeCars[otherVId] and 0.8 or 0.5), otherVeh.jbeam == "unicycle")
      ::continue::
    end
  end
end
