...
local M = {}
M.dependencies = {"ui_apps_minimap_topomap", "ui_apps_minimap_additionalInfo", "ui_apps_minimap_route", "ui_apps_minimap_utils", "ui_apps_minimap_roads", "ui_apps_minimap_vehicles"}
M.onInit = function()
...
-- unoptimized
  ...
  td:circle(drawX, drawY, circleRadius, 3, ui_apps_minimap_utils.colors.grayMuted , ui_apps_minimap_utils.colors.grayMuted, ui_apps_minimap_utils.colors.navBgWhite, ui_apps_minimap_utils.colors.navBgWhite)
  ...
  td:line(leftTop.x, leftTop.y, leftBottom.x, leftBottom.y, lineWidth, lineWidth, 0, 0, strikeColor, strikeColor, strikeColor, strikeColor)
  td:line(rightTop.x, rightTop.y, rightBottom.x, rightBottom.y, lineWidth, lineWidth, 0, 0, strikeColor, strikeColor, strikeColor, strikeColor)
  td:line(diagonalTop.x, diagonalTop.y, diagonalBottom.x, diagonalBottom.y, lineWidth, lineWidth, 0, 0, strikeColor, strikeColor, strikeColor, strikeColor)
...
local function drawMinimap(sizeX, sizeY, dtReal, dtSim)
  ...
  if debugSettings.draw100m then
    local w = 20
    td:circle(width/2 + offsetX,height/2 + offsetY, 100/scale-w, w, 0, 0, color(255,255,255,0),color(255,255,255,250))
    if p then p:add("100mRadius") end
  end

  -- draw playmode markers
  if debugSettings.drawPlaymodeMarkers and not gameplay_missions_missionManager.getForegroundMissionId() then
    drawPlaymodeMarkers()
  ...
  extensions.hook("onDrawOnMinimapBorder", td)
end
...
M.drawOcclusion = function(offsetX, offsetY)
  ...
  local dpi = im.GetWindowDpiScale()
  --td:circle(width/2 + offsetX-30, height/2 + offsetY, 50*dpi,20*dpi, white, white, transparent, transparent, flag)
  --td:circle(width/2 + offsetX+30, height/2 + offsetY, 50*dpi,20*dpi, white, white, transparent, transparent, flag)
  ...
  for _, transform in pairs(occlusionTransforms) do
    ...
    td:line(left, transform[6]+h/2, right, transform[6]+h/2, h/2-5, h/2-5, 5, 5, transparent, transparent, white, white, flag)
  end
  ...
  if debugSettings.drawOcclusion then
    for id, transform in pairs(occlusionTransforms) do
      ...
      local left, right, h = transform[5]+5, transform[7]-5, transform[8]-transform[6]
      td:line(left, transform[6]+h/2, right, transform[6]+h/2, h/2-5, h/2-5, 5, 5, color(255,0,0,128), color(255,0,0,128), color(255,0,0,255), color(255,0,0,128))
    end
  end
  ...
  if mode == "circle" then
    -- crop to circle
    td:circle(width/2 + offsetX, height/2 + offsetY, width/2, width/2, white, white, transparent, transparent, flag)
    td:circle(width/2 + offsetX, height/2 + offsetY, width/2-10*dpi,   12*dpi, white, white, white, transparent, flag)
    if debugSettings.drawOcclusion then
      td:circle(width/2 + offsetX,height/2 + offsetY, width/2, 20, color(255,0,0,128), color(255,0,0,128), color(255,0,0,255), color(255,0,0,128))

      --td:circle(width/2 + offsetX,height/2 + offsetY, width/2-10*dpi, 11*dpi, color(255,0,0,128), color(255,0,0,128), color(255,0,0,255), color(255,0,0,128))

    end
  end
  if mode == "rect" then

    td:line(bufferLeft+buffer*1.5, bufferTop+height/2, width+bufferLeft-buffer*1.5, bufferTop+height/2, height/2-buffer*2, height/2-buffer*2, roundness, strikeWidth, white, white, transparent, transparent, flag)

    roundness, strikeWidth = 5, 10
    buffer = 5

    td:line(bufferLeft+buffer*1.5, bufferTop+height/2, width+bufferLeft-buffer*1.5, bufferTop+height/2, height/2-buffer*2, height/2-buffer*2, roundness, strikeWidth, white, white, white, transparent, flag)
  end
...
M.drawDebugOverlay = function()
  ...
  td:circle(0,0, 5,5, 0, 0, color(0,255,0,255), color(0,255,0,255))
  td:circle(texWidth ,0, 5,5, 0, 0, color(0,255,0,255), color(0,255,0,255))
  td:circle(0,texHeight , 5,5, 0, 0, color(0,255,0,255), color(0,255,0,255))
  td:circle(texWidth ,texHeight , 5,5, 0, 0, color(0,255,0,255), color(0,255,0,255))


  td:circle(bufferLeft ,bufferTop, 5,5, 0, 0, color(255,255,255,255), color(255,255,255,255))
  td:circle(width + bufferLeft ,bufferTop, 5,5, 0, 0, color(255,255,255,255), color(255,255,255,255))
  td:circle(bufferLeft ,height + bufferTop, 5,5, 0, 0, color(255,255,255,255), color(255,255,255,255))
  td:circle(width + bufferLeft ,height + bufferTop, 5,5, 0, 0, color(255,255,255,255), color(255,255,255,255))

  td:circle(bufferLeft/2, texHeight/2, bufferLeft/2, 1, 0, 0, color(255,0,0,255), color(255,0,0,255))
  td:circle(texWidth/2,bufferTop/2, bufferTop/2, 1, 0, 0, color(255,0,0,255), color(255,0,0,255))
  td:circle(texWidth-bufferRight/2,texHeight/2, bufferRight/2, 1, 0, 0, color(255,0,0,255), color(255,0,0,255))
  td:circle(texWidth/2,texHeight-bufferBottom/2, bufferBottom/2, 1, 0, 0, color(255,0,0,255), color(255,0,0,255))
