...
function C:init(parent)
  ...
  self.modules = { }
  for _, m in ipairs({'missionReplay', 'drift', 'vehicle', 'level', 'prefab', 'timer', 'button', 'action', 'camera', 'file', 'traffic', 'mission', 'foreach', 'thread', 'ui'}) do
...
function C:getAvailableMacros()
  local macroPath = '/flowgraphEditor/macros/'
  local lookup = {}
...
function C:_onDeserialized(data)
  ...
  if next(data) then
    ...
    if data.version == 0.1 then
      log("I", logTag, "This is an older version (" .. data.version .. " vs " .. ui_flowgraph_editor.flowgraphVersion .. "). Updating format.")
      ...
      while doFlatten do
        ...
        for gId, g in pairs(currentGraphs) do
          for cId, c in pairs(g.children or {}) do
            ...
            log("I", logTag, "Flattened child: " .. gId .. "/" .. cId)
          end
          g.children = nil
          newGraphs[gId] = g
        end
    ...
    if not self.stateGraph then
      log("D", logTag, "Auto-Generating stategraph.")
...
function C:setRunning(running, stopInstant)
  ...
  if running then
    ...
    if editor and editor.getPreference and editor.getPreference("flowgraph.debug.debugGarbage") then
      log("I", logTag, "Garbage Debug is enabled for Flowgraph " .. self.name .. " (" ..self.id..")")
    ...
    if oldState ~= 'stopped' then
      ...
      if stopInstant then
        log("W", logTag, "Stopping project instantly!")
        self:_executionStopped()
        --self:_onClear()
        self:destroy()
      else
...
function C:blocksOnResetGameplay()
  ...
  if self.blocksOnResetGameplayCache == nil then
    for node in self:allNodes() do
      if (node.data and node.data.blocksOnResetGameplay) or node.blocksOnResetGameplay then
