  ...
  for _, m in pairs(self.minimapMarkers or {}) do
    ...
    if m.left and m.right then
      if m.mode ~= 'hidden' then
        ui_apps_minimap_utils.simpleLine(m.left, m.right, clr, color(255,255,255,192))
      end
    else
      if m.mode ~= 'hidden' then
        ui_apps_minimap_utils.simpleCircle(m.pos, clr, color(255,255,255,192))
      end
    end
  end
end
...
function C:onDrawOnMinimapBorder(td)
  ...
  for _, m in pairs(self.minimapMarkers or {}) do
    local clr = m.color
    if m.mode ~= 'hidden' then
      ui_apps_minimap_utils.drawEdgePointer(m.pos, clr, m.mode == 'hidden' and color(255,255,255,192) or nil)
    end
  end
end
