...
local function initAdasUpdate()
  if electrics.values.wheelspeed < 0.02 then
    extensions.tech_adasInput.applyAdasInput(0, 'brake')
    extensions.tech_adasInput.applyAdasInput(100, 'throttle')
    lastBrake = 0
    return
...
local function applyBrakes(dist)
  ...
  if brake > 0.01 then
    extensions.tech_adasInput.applyAdasInput(brake, 'brake')
    extensions.tech_adasInput.applyAdasInput(0, 'throttle')
    lastBrake = brake
  else
    extensions.tech_adasInput.applyAdasInput(0, 'brake')
    extensions.tech_adasInput.applyAdasInput(1, 'throttle')
    lastBrake = 0
  end
end
