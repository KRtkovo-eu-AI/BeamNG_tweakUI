...
local function update(cylinder, inputPressure, dt)
  ...
  valveDirection = valveDirection * cylinder.direction or 1
  ...
  local cylinderReliefSlipSpeedLimit = cylinder.cylinderReliefSlipSpeedLimit
  ...
  if cylinder.valveType == "drop" then
    ...
    if valveDirection < cylinder.dropInputThreshold then
      local dropInput = linearScale(abs(valveDirection), abs(cylinder.dropInputThreshold), 1, 0, 1)
      --slipForce = linearScale(dropInput, 0, 1, slipForce, 0)
      slipForce = 0
      cylinderReliefSlipSpeedLimit = linearScale(dropInput * dropInput, 0, 1, 0, cylinderReliefSlipSpeedLimit)
      cylinderForce = 0
    end
  ...
  for i = 1, cylinder.beamCount do
    ...
    local beamVelocity = obj:actuateBeam(cid, cylinderForce, speedLimit, slipForce, dragForce, cylinderReliefSlipSpeedLimit, cylinder.minExtend, cylinder.maxExtend)
